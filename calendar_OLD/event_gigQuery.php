<?php
		...
		/* Query database for gigs accepted by artists */
			...
			$gigAccepted='SELECT gigartists.gigId, gigdetails.gigDetails_gigName, gigdetails.gigDetails_setupTime, gigdetails.gigDetails_endTime
					     ...
					     INNER JOIN gigstatus on gigartists.gigId = gigstatus.gigId
					     WHERE gigartists.gigArtists_userId = ? AND gigartists.gigArtists_artistStatus = ? AND gigartists.gigArtists_gigManCancelStatus = ? AND gigstatus.gigStatus_status = ? AND';
			if($privSetting == 'pub') {
			...
			try {
				...
				echo 'There was an error Retrieving Gig Managers Information<pre>'; 
				echo $e; 
			}
		...
		/* Query database for gigs created by users */
			$gigManStat1 = 'active';
			$gigCreated='SELECT gigmaster.gigId, gigdetails.gigDetails_gigName, gigdetails.gigDetails_setupTime, gigdetails.gigDetails_endTime 
					 	 FROM gigmaster 
					  	 INNER JOIN gigdetails on gigmaster.gigId = gigdetails.gigId
					     INNER JOIN gigstatus on gigmaster.gigId = gigstatus.gigId
					     WHERE gigmaster.gigMaster_loginId = ? AND gigstatus.gigStatus_status = ? AND ';  
			if($privSetting == 'pub') {
	...
	/* Update arrays for all gig, gig manager, and recreational events for user in preperation for JSON Feed 
		...
		*/	if($privSetting == 'all') {
				$urlArt = URL . 'managebooking/artistInnerGigB.php?g_Id=';
				$urlMan = URL . 'manageBooking/innerGigPage.php?g_Id=';
