...
/**** Send ajax request to retrieve gig posts from specific states ****/
	$("#postDisplay").on("click", '.getCities', function(event){
		...
		if(state != ''){
			$.ajax({
				url: "https://www.stage.gospelscout.com/findgigs/phpBackend/g_postedGigs.php?getPosts=true&state="+state,
				type: "GET",
				statusCode: {
				    404: function() {
...
/**** Send ajax request to retrieve gig posts from specific cities ****/
	$("#postDisplay").on("click", '.getGigs', function(event){
		...
		/**** If the city var is not empty send AJax requery to get gigs for that city ****/
			$.ajax({
				url: "https://www.stage.gospelscout.com/findgigs/phpBackend/g_postedGigs.php?getPosts=true&state="+state+"&city="+city, 
				type: "GET",
				statusCode: {
				    404: function() {
				      alert( "page not found" );
...
/**** Get the filterred search form ****/
	$('#filteredSearch').click(function(event){
		...
		/* Send Ajax request */
			$.ajax({
				url: "https://www.stage.gospelscout.com/findgigs/phpBackend/g_postedGigs.php",
				method: "POST",
