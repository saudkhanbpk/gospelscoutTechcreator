...
function displayGigInfo(gigsobj){
  ...
  for(x in gigsobj.gigs){
    codeInsert += '<tr><td class="d-none d-md-table-cell">'+gigsobj.gigs[x].gigId+'</td><td class="d-none d-md-table-cell">'+gigsobj.gigs[x].gigType+'</td><td>'+gigsobj.gigs[x].eventdate+'</td><td>'+gigsobj.gigs[x].sGiftName+'</td><td class="d-none d-md-table-cell">'+gigsobj.gigs[x].age_of_post+'</td><td><a href="https://www.gospelscout.com/publicgigads/ad_details.php?g_id='+gigsobj.gigs[x].gigId+'" target="_blank" class="text-gs viewEventDeets" id="show_gig_deets" eventTable="'+gigsobj.gigs[x].eventtable+'" rowEntryID="'+gigsobj.gigs[x].id+'">View</a></td></tr>';
  }
    ...
    /* Build the Gig Ad Table */ 
		...
		if( gigsobj.gigs[status]){
			...
			codeInsert += '<thead><tr><th class="d-none d-md-table-cell">Event ID</th><th class="d-none d-md-table-cell">Event Name</th><th>Event Type</th><th>Event Date</th><th class="d-none d-md-table-cell">Viewable</th><th>View Details</th></tr></thead><tbody>';
				...
				codeInsert += '<tr><td class="d-none d-md-table-cell">'+gigsobj.gigs[status][x].gigId_display+'</td><td class="d-none d-md-table-cell">'+gigsobj.gigs[status][x].gigName+'</td><td>'+gigsobj.gigs[status][x].gigType+'</td><td>'+gigsobj.gigs[status][x].gigDate+'</td><td>'+viewable+'</td><td><a href="https://www.gospelscout.com/publicgigads/ad_details.php?g_id='+gigsobj.gigs[status][x].gigId+'" target="_blank" class="text-gs">View</a></td></tr>'; //eventTable="'+gigsobj.gigs[x].eventtable+'"
				}
...
/* fetch users Gigs */
  function fetchGigs(gig_status){
      /* Instantiate new XHR to fetch gigs */
        ...
        getGigs_xhr.open('get','https://www.gospelscout.com/gigmanager/phpBackend/connectToDb.php?gig_status='+gig_status+'&1'); //+'&iLoginID='+u_id
        getGigs_xhr.send(); 
  }
...
/* Fetch user's gig ads */
  function fetchGigAds(status){
    ...
    /* Grab user's gig ads */
      ...
      get_gigAds_xhr.open('get','https://www.gospelscout.com/gigmanager/phpBackend/connectToDb.php?eventTable='+eventTable+'&gigAds=true');
      get_gigAds_xhr.send(); 
  }
