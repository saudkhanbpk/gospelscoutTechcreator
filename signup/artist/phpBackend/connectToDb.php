<?php 
	...
	/* Check that email is not used */
		if($_GET['loginmaster']['sEmailID'] !== '' && $_GET['loginmaster']['sEmailID'] !== NULL){
			...
			$cond0="sEmailID = '".$sEmailID0."' AND (sUserType = 'artist' OR sUserType = 'gen_user' OR sUserType = 'group') AND isActive = 1";
				...
				if(count($loginrow) > 0){				
					...
					/****************************** Handle $_POST Array Validation ***********************************************/
						...
						/* Implement zipcode api to return city based on zip code */
							...
							if($errorCount == 0){
								if($state !== $apiState){
									$errorMessage = 'State/Zip code Mismatch'.$state.' '.$apiState;
									$errorCount++;
								}
								else{ 
									$cond = 'name = ' . $state;
			...
			if($errorCount > 0 ){  
				...
				if($iLoginID > 1 ){ // loginmaster conditonal 
					...
					if($iUserID > 1){ // usermaster conditional
						...
						if($talID > 1){ // talentmaster conditional
												...
												/* 1. We Create connect account first */
													...
													/*************************** Create EXPRESS CONNECT ACCTS ***************************/
														/* Link for express account onboarding */
															...
															// $bmonth = date_format($bdate, 'n');
															// $byear = date_format($bdate, 'Y');
															// $url = 'https://dev.gospelscout.com/newHomePage/views/artistprofile.php?artist=' . $newUser['iLoginID']; 
															// $client_id = 'ca_Ak0oINUTgAn12kLMbPdMh2PuNJuXU11L';

															// $additional_parameters = array(
															// 		"redirect_uri" => 'https://dev.gospelscout.com/newHomePage/views/artistprofile.php',
															// 		"client_id" => $client_id,
															// 		"state" => $apiState,
														...
														// if($_POST['skipStripeSetup']){
															/*********** Upon successful profile submission, relocate user to their profile page ************/
															    $responseObj['target_url'] = 'https://www.gospelscout.com/views/artistprofile.php';
