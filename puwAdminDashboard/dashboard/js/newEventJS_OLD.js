			...
			/* Submit new event */
				if(hostID > 0){
					...
					/* New XMLHttp Obj */
						...
						newEvent_xhr.open('post','https://www.gospelscout.com/puwAdminDashboard/dashboard/phpBackend/connectToDb.php');
	...
	/* Show 'add new host' modal */
		$('.getEvent').click(function(event){
			...
			/* Fetch Event info */
				...
				fetchEvent_xhr.open('get','https://www.gospelscout.com/puwAdminDashboard/dashboard/phpBackend/connectToDb.php?fetchEvent='+eventID);
				fetchEvent_xhr.send();
		});
/*************************
*** END - view/Edit Event
...
*************************/
	$('#cancelEventButton').click(function(event){
		...
		/* Instantiate new xhr */
			...
			cancEvent_xhr.open('post','https://www.gospelscout.com/puwAdminDashboard/dashboard/phpBackend/connectToDb.php');
...
*************************/
		...
		/* Get Hosts for the selected state */
			if(state !== ''){
				...
				/* Fetch Hosts for the selected city */
					...
					getHost_xhr.open('get','https://www.gospelscout.com/puwAdminDashboard/dashboard/phpBackend/connectToDb.php?get_hosts_in_state=true&state='+state);
					getHost_xhr.send(); 
			}
			else{
	...
	/* Place selected host in event form */
		$("#accordionExample").on("click", '#sel-host-button', function(event){
			...
			hostById_xhr.open('get','https://www.gospelscout.com/puwAdminDashboard/dashboard/phpBackend/connectToDb.php?get_host_by_id=true&host_id='+host_id);
			hostById_xhr.send();
		}); 
	...
	$('#selectArtistButton').click(function(event){
		...
		/* Fetch all artitsts apply to perform at a popUpWorhsip event in the specified city */
			if(state !== '' && talent_id !== ''){
				...
				getArtists_xhr.open('get','https://www.gospelscout.com/puwAdminDashboard/dashboard/phpBackend/connectToDb.php?get_artists_by_state=true&state='+state+'&talent_id='+talent_id);
				getArtists_xhr.send(); 
			}else if(state == ''){
