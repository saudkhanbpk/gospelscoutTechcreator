...
<?php
	...
	/* Get the user's subscriptions from the subscriptions table */
		$type = 'artist'; 
		$isActive = '1';
		$cond = 'iLoginID = ' . $currentUserID . ' AND (sType = "artist" OR sType = "group")' . ' AND isActive = 1';
			...
			<!-- Gig Manager Playing Status -->
				<?php 
					if($currentUserType == 'group'){
						/* Do not display any thing */
					}
					elseif($currentUserType == 'artist' && $user == 'manager'){?>
						...
						<div class="row pl-2">
						    ...
						    <div class="col-10 col-md-7">
						     ...
						     <?php if($user == 'manager' && $form == 'existing' && $gigState == 'active'){?>
								...
								<button class="btn-sm btn-gs" type="button" data-toggle="modal" data-target="#sendClientEmail">Send Client Invoice Email</send>
							     </div>
						     <?php }?>
						</div>
						<hr class="mt-4"> <!-- Page Divider -->
									...
									<table class="table table-striped table-font-size">
										...
										<tbody>
											...
											foreach($artistList as $artist){?>
													<tr class="text-gs">
														<td><a class="text-gs" href="<?php echo URL;?>views/artistprofile.php?artist=<?php echo $artist['gigArtists_userId'];?>"><?php echo ucwords($artist['gigArtists_name']);?></a></td>
...
<!-- Send Client invoice Email -->
	<form action="" method="POST" name="sendClientEmail" enctype="multipart/form-data">
		<!-- Modal Send Client gig invoice -->
			<div class="modal fade" id="sendClientEmail" tabindex="-1" role="dialog" aria-labelledby="sendClientEmail" aria-hidden="true">
			  <div class="modal-dialog modal-dialog-centered" role="document">
			    <div class="modal-content">
			    	<!-- Error Message Display -->
					    <div class="container p-3 text-center mb-0 d-none" id="sendClientEmail-error-message">
					      	<p class="m-0 p-2 card-shadow text-white bg-gs" id="sendClientEmail-error-text" style="border-radius:7px"></p>
					    </div>
					<!-- /Error Message Display --> 

					<!-- Modal Title -->
					    <div class="modal-header">
					        <h5 class="modal-title" id="exampleModalLongTitle">Send Client Email</h5>
					        <button type="button" class="close" data-dismiss="modal" aria-label="Close" onclick="sendClientEmailReset()">
					        	<span aria-hidden="true">&times;</span>
					        </button>
					    </div>
				    <!-- /Modal Title -->
				    ...
				    <!-- Modal Body -->
				      	<div class="modal-body">
					    	<div class="form-group">
					    		<label for="vidTalent">Additional Comments</label>
								<textarea class="form-control mb-2" name="clientEmail_comment" placeholder="Write additional comments..." wrap="" rows="4" aria-label="With textarea"></textarea>
								<input type="hidden" name="gigId" value="<?php echo $_GET['gigID'];?>">
								<input type="hidden" name="gigManager" value="<?php echo $userInfo['sFirstName'] . ' ' . $userInfo['sLastName'];?>">
								<input type="hidden" name="gigManager_userID" value="<?php echo $currentUserID;?>">
								<input type="hidden" name="gigManager_email" value="<?php echo $currentUserEmail;?>">

								<!--
								<div class="ml-3">
								 	<h6>Include the following additional gig info:</h6>
								 	<ul>
								 		<?php 
								 			$moreInfo = 0;
								 			if($getGigMusic){
								 		...
								 		?>
								 				<li>
										 			<input class="form-check-input ministries" name="gigmusic" type="checkbox"  value="1">
													<label class="form-check-label" for="gigmusic"> Gig Music</label>
										 		</li>
										<?php }
											$artConfCount = 0;
											foreach($getGigArtists as $gigArtist){
												if($gigArtist['gigArtists_gigManCancelStatus'] == 'active' && $gigArtist['gigArtists_artistStatus'] == 'confirmed'){
													$artConfCount++; 
												}
											}	
										...
										?>
										 		<li>
										 			<input class="form-check-input ministries" name="gigartists" type="checkbox"  value="1">
													<label class="form-check-label" for="gigartists"> Gig Artists</label>
										 		</li>
										<?php }
											if($moreInfo == 0){
										?>
												<li style="list-style:none;color: rgba(204,204,204,1)">No additional gig info available.</li>
										<?php
											}
										?>
								 	</ul>
				    ...
				    <!-- Modal Footer -->
				      	<div class="modal-footer">
				        	<button type="button" class="btn btn-gs" data-dismiss="modal" aria-label="Close" id="clientEmailButton">Send Email</button>
				        	<button type="button" class="btn btn-gs" data-dismiss="modal" aria-label="Close" onclick="sendClientEmailReset()">Cancel</button>
				      	</div>
				    <!-- /Modal Footer -->
			    </div>
			  </div>
		    	...
		    	<div class="form-group">
		    		<form action="" method="post" name="addSubArt" id="addSubArt" class="">
		    			...
		    			<div class="container" style="font-size:14px">
		    				<ul style="list-style:none">
		    					<li>
		    						...
		    						<label>
		    							<?php 
			    							if($j > 0){
		    									echo '<a class="text-gs font-weight-bold" href="' . URL . 'views/artistprofile.php?artist=' . $sub['iRollID'] . '" target="_blank"><del>' . $sub['sName'] . '</del></a>';
		    								}
		    								else{
		    									echo '<a class="text-gs font-weight-bold" href="' . URL . 'views/artistprofile.php?artist=' . $sub['iRollID'] . '" target="_blank">' . $sub['sName'] . '</a>';
		    								}
		    							?>
	    						</label>
							...
							<input type="hidden" name="<?php echo $sub['iRollID'];?>[gigArtists_userType]" value="<?php echo $sub['sType'];?>">
		    								...
		    								foreach($subscripTalent as $index => $indivSub){
		    									if($index == $sub['iRollID']){
													...
													echo '<li style="font-size: 12px">';
							    						...
							    						echo '	<lable>' . str_replace("_","/",$talents['talent']) . '</label>';
							    						echo '</li>';
												}
		    									}
										}
...
<script>
		...
		function sendSubArt(){
			...
			addSubArt.onreadystatechange = function(){
				if(addSubArt.readyState == 4 && addSubArt.status == 200){
					...
					var trimmedText = addSubArt.responseText.trim();
					if(trimmedText == 'inserted'){
						$('.subTalList').addClass('d-none');
								...
								if(findArtist.status == 200 && findArtist.readyState == 4){
									...
									if(trimText == 'noResult'){
										...
										/* Define Variables from returned xml values */	
											...
											if(talent == 'group'){
												var fullName = xmlDoc.getElementsByTagName('gname')[0].childNodes[0].nodeValue; 
											}
											else{
	...
	/******************************** Send Client Email *********************************************/
		$('#clientEmailButton').click(function(){
			/* Get Client email form data */
				var sendClientForm = document.forms.namedItem('sendClientEmail');
				var sendClientObj = new FormData(sendClientForm);
			/* END - Get Client email form data */

			/* Send client email data to th send sendClientEmail page via XMLHttpRequest */
				var sendClientEmail = new XMLHttpRequest(); 
				sendClientEmail.onreadystatechange = function(){
					if(sendClientEmail.status == 200 && sendClientEmail.readyState == 4){
						//console.log(sendClientEmail.responseText);
						document.forms.namedItem('sendClientEmail').reset();
					}
				}
				sendClientEmail.open('POST', '<?php echo URL;?>views/sendClientEmail.php'); 
				sendClientEmail.send(sendClientObj); 
			/* END - Send client email data to the send sendClientEmail page via XMLHttpRequest */
		});

		/* Clear send client email form */
			function sendClientEmailReset(){
				document.forms.namedItem('sendClientEmail').reset();
			}
		/* END - Clear send client email form */


	/***************************** END - Send Client Email ******************************************/
</script>
