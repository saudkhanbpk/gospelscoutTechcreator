			...
			if($_GET['likes'] || $_GET['c_likes']){
				if($_GET['likes']){
					/* Like-Button functionality 4 artist videos */
						...
						$paramlikes['id']['='] = $_GET['vidID'];
						$columnlikes = array('videoLikes');
						$likes = pdoquery($table,$columnlikes,$paramlikes,$emptyArray,$emptyArray,$emptyArray,$emptyArray,$emptyArray,$emptyArray);

						$newLikes = $likes[0]['videoLikes'] + 1;
						// $value = array($newLikes);
						// $obj->update($field,$value,$cond,$table);
						$arrayUpdate = array('videoLikes' => $newLikes);
						$updateSuccess = updateTable($db, $arrayUpdate, $cond, $table);

						// $nlikes = $obj->fetchFieldAll($table,$field,$cond);
						// $nlikes = pdoquery($table,$columnlikes,$paramlikes,$emptyArray,$emptyArray,$emptyArray,$emptyArray,$emptyArray,$emptyArray);
			...
			elseif($_GET['views'] || $_GET['c_views']){
				if($_GET['views']){
					/* # of artist video Views Display Functionality */
						...
						$paramViews['id']['='] = $_GET['vidID'];
						$columnViews = array('videoViews');
						$views = pdoquery($table,$columnViews,$paramViews,$emptyArray,$emptyArray,$emptyArray,$emptyArray,$emptyArray,$emptyArray);
						
						$newViews = $views[0]['videoViews'] + 1;
						$cond = 'id = ' . $_GET['vidID'];
						$arrayUpdate = array('videoViews' => $newViews);
						updateTable($db, $arrayUpdate, $cond, $table);

						$nViews = pdoquery($table,$columnViews,$paramViews,$emptyArray,$emptyArray,$emptyArray,$emptyArray,$emptyArray,$emptyArray);
			...
			elseif($_GET['getComments']){
				...
				if($_GET['repliesTo'] || $_GET['c_repliesTo']){
					...
					if($_GET['repliesTo']){
						...
						$commentQuery = 'SELECT videocommentreplies.*, usermaster.sProfileName, usermaster.sFirstName, usermaster.sLastName, usermaster.sUserType
...
?>							
									<reply>
										...
										<sUserType><?php echo $eachReply['sUserType'];?></sUserType>
									</reply>
<?php					
								}	
							echo '</replies>';
...
<?php					
			...
			elseif($_GET['deleteReply']){
				...
				if($_GET['deleteReply'] == 'artist' || $_GET['deleteReply'] == 'group'){
					/* Remove $_GET['deleteComment'] from $_GET aray */
		...
		elseif(!empty($_POST)){
			...
			elseif($_POST['replyType']){
				...
				if($_POST['replyType'] == 'artist' || $_POST['replyType'] == 'group' || $_POST['replyType'] == 'church'){
					/* Remove Marker */
