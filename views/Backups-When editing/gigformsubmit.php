<?php 
	...
	if($_POST){
		...
		/************************************************** Define email Variables **************************************************/
			...
			$requestorUrl = 'https://dev.gospelscout.com/newHomePage/views/artistprofile.php?artist=' . $emailParamResults['gigDetails_gigManLoginId']; 
			...
			/* Query the database for all artists associated with the current gig id and define email vars for targeted artists */
				if($action == 'insert' && $table == 'gigartists'){
					...
					$talentRequested = $POST['gigArtists_tal'] ;
				}
				else{
					$gmCancStatus = 'active';
					$gaStatus1 = 'pending';
		...
		if($_POST['srcPage'] == 'artistProfile' && $action == 'insert'){
			...
			/* Insert artists info and request info into their repsective tables */
				...
				/*
				var_dump($detailsField);
				var_dump($detailsValue);
				var_dump($requestsField);
				var_dump($requestsValue);
				var_dump($artistsField);
				var_dump($artistsValue);
				exit;
				*/
				if($gigDeets){
					...
					/*Check for successful update to the database */
						if($updateSucc && !($POST['artistViewed'])){
							...
							/* Trigger email to artists */
								if($POST['gigDetails_cancelReason']){
