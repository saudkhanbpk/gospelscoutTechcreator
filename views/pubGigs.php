...
<form  action="<?php echo URL;?>views/xmlhttprequest/pubGigBackbone.php" method="post" name="gigdetails" id="gigdetails" class="needs-validation mt-4 gigEdit pb-4 pt-2" table="gigdetails" novalidate>
	<div class="container bg-white mt-0 mt-md-5 mb-0 mb-md-3 px-md-5 py-3" style="max-width:900px;min-height:700px">
	...
	<!-- /Venue Info -->
		...
		<!-- General Gig Request for gigs poseted for public inquiry -->
			<div class="container mt-2"> <!-- common section -->
				<h5 class="text-gs text-center text-md-left">Additional Requests/Requirements</h5>
				<div class="row">
					<div class="col">
		...
		<?php 
			if($updating){
				...
				echo '<button class="btn btn-small btn-gs text-white getStatus" type="submit" >Post Gig</button>'; //get-modal="" data-toggle="modal" data-target=""  id="postGig"
			}
		?>
                        ...
                        <div class="carousel-inner">
                            <div class="carousel-item bg-white active" style="max-height:325px;overflow:hidden;overflow:scroll">
                                <div class="container">
                                    ...
                                    <div class="row">
                                        <div class="col">
                                            <h6>Your first <span class="font-weight-bold text-gs">Month</span> of posting is on us!!!</h6>
                                            <p class="pl-2" style="font-size:12px"> From the date and time that you create your profile, you will have 30 days of free posting so go nuts!!!  After that, we'll be watching.</p>
                                            <h6>Only pay for what you use!!!</h6>
                                            <p class="pl-2" style="font-size:12px"> You are never charged for having an account with us.  You only pay for what you use.  We track your usage through out the month and add up your total usage at the end of your billing cycle. Keep in mind, the more you post, the cheaper it is.  Check out our pricing below.</p>
                                            <h6>The more you post the cheaper it is!!!</h6>
                                            <ul class="pl-2">
                                                <li>1-4 posts/month: $4.00/posts</li>
                                                <li>5+ posts/month: $2.00/posts</li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                ...
                sendCust_info.onreadystatechange = function(){
                    if(sendCust_info.readyState == 4 && sendCust_info.status == 200){
                        ...
                        if(parse_respTxt['trial_end'] == false){
                            var trialEnd = '<p class="font-weight-bold">Your posts are on us until '+parse_respTxt['exp_date_time']+'.  Happy Gigging!!!</p>';
              ...
              $.support.cors = true;
                  ...
                  $.getJSON('https://maps.googleapis.com/maps/api/geocode/json?address=' + userZip + '&key=AIzaSyCRtBdKK4NmLTuE8dsel7zlyq-iLbs6APQ&type=json&_=' + date.getTime(), function(response){
                    ...
                    if(state == userState){
					    ...
					    if(time.indexOf(' PM')  != -1 && hours < 12) {
					        ...
					        time = time.replace(hours, (hours2 + 12));
					    }
				...
				/* END - Convert the time into format useable by date() function */	
                        ...
                        /* Change format of the times */
                        	...
                        	var todayD = new Date();
                        	var enteredD = new Date($('input[name=gigDate]').val()+'T'+sTime); 
