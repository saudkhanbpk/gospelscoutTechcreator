			...
			if(count($loginrow) > 0){				
				...
				/****************************** Handle $_POST Array Validation ***********************************************/
					...
					/* Birth Date Validation */
						...
						if($diff->y < 18){
							$errorMessage = 'Must be at least 18yrs. of age to create a church profile';
							$errorCount++;
						}
			...
			if($errorCount == 0){
				...
				if($iLoginID > 0){ // loginmaster conditional
					...
					if($iUserID > 0){ // usermaster conditional
						...
						/******************** Create Subscribe table entries for new users *********************/
							$field = array("iRollID"); 
						...
						/***************** END - Create Subscribe table entries for new users ******************/
							...
							if($subSett1 > 0 && $subSett2 > 0){ // subscribee & subscriber table conditional
								/******************** Move Profile img to permanent location on the server *********************/
									...
									/* PROCESS UPLOADED Profile IMG */
										$musicDir = '/upload/church/'.$iLoginID.'/'; 
										...
										if(count($newIndivFile) > 0){
											...
											elseif($musicFileSize != '' && $musicFileSize > 5000000) {
												...
												/* Add profile img File Path to the post array */
													$profPic_dir = $musicDir . basename($target_file_mus);
											}
										} 
									...
									if($uploadMusStatus == 'upload'){
										...
										/* Update usermaster table with the profile img location */
											$condUpD = 'iLoginID = '.$iLoginID;
											$tableUpD = 'usermaster';
											$fieldUpD = array('sProfileName');
											$valueUpD = array($profPic_dir);
											$obj->update($fieldUpD,$valueUpD,$condUpD,$tableUpD);
									}	
									/*if($uploadMusStatus = 'upload'){
										if(file_exists($target_dir)) {
											moveFile($newIndivFileMus, $target_file_mus);
								...
								/*********************** Create a Stripe Customer object for the new user ***********************/
									...
									if(count($newUser) > 0){
										/**************************************************** create a stripe customer ****************************************************/
											try{
												$inv_pref = ucfirst(substr($newUser['sFirstName'],0,1)).ucfirst(substr($newUser['sLastName'],0,1)).$iLoginID; //(users first initial+last initial+user_ID)
												$customerObj = \Stripe\Customer::create([
													...
													"metadata" => [
														"first_name" => ucfirst($newUser['sFirstName']),
														"Last_name" => ucfirst($newUser['sLastName']),
														"user_ID" => $iLoginID
													],
												]);
											}catch(Exception $e){
										...
										/************************************************** Create A Stripe Subscription **************************************************/
											if($upD_cust){
												...
												$errorMessage = 'Trouble Updating table w/ Str_Customer ID';
											}
										/*********************************************** END - Create A stripe Subscription ***********************************************/

										/******************* Insert New into the churchministrymaster and churchamenitymaster tables *******************/
											$fieldMinistry = array('iLoginID'); 
											$valueMinistry = array($iLoginID);
											$tableMinistry = 'churchministrymaster'; 

											$fieldMinistry2 = array('iLoginID','iGiftID','sGiftName'); 
											$valueMinistry2 = array($iLoginID,'15','Preaching_Teachings');
											$tableMinistry2 = 'churchministrymaster2'; 

											$fieldAmenity = array('iLoginID');
											$valueAmenity = array($iLoginID);
											$tableAmenity = 'churchamenitymaster';

											$insertSub = $obj->insert($fieldMinistry,$valueMinistry,$tableMinistry);
											$insertSub = $obj->insert($fieldMinistry2,$valueMinistry2,$tableMinistry2);
											$insertSub = $obj->insert($fieldAmenity,$valueAmenity,$tableAmenity);
										/**************** END - Insert New into the churchministrymaster and churchamenitymaster tables ****************/
...
<div id="carouselExampleIndicators" class="carousel mb-2 pt-0 pb-0 mt-5 text-center valid-form-check bg-bg2" style="background-image: url(../img/BusinessCardCollage2.png);" data-wrap="false" data-interval="false" data-ride="false"> <!-- carousel-->
	 <div  class="pt-5" style="width:100%; height:100%;background-color: rgba(149,73,173,.3);">
		...
		<h4 class="mt-4 text-white">Church Sign Up</h4>
					
		<div class="carousel-inner ">
			<div class="container px-md-5" >
				<form action="church.php" method="post" name="artist-signup" id="artist-signup" enctype="multipart/form-data" autocomplete="off">
					<!-- Slide 1 -->
					    	...
					    	<div class="container pt-2" >
					    		<div class="row pt-3 justify-content-center">
					    			<div class="col col-md-5 pt-3 pb-5 bg-white signup-steps" style="max-height: 475px;overflow:scroll;">
					    				...
					    				<div class="text-left text-black font-weight-bold">Profile Admin's Info</div>
					    	...
					    	<div class="container pt-2">
					    		<div class="row pt-3 justify-content-center">
					    			<div class=" col col-md-5 text-center bg-white pt-3 pb-5 signup-steps" style="max-height: 475px;overflow:scroll;">
					    				...
					    				<div class="text-left text-black font-weight-bold mb-3">Profile Admin's Info</div>
					    				<!-- Security Questions -->
										    ...
										    <div class="container m-0 mb-2 pl-2">
											    <input type="text" autocomplete="off" style="max-width:300px;" class="mt-1 form-control form-control-sm clearDefault mt-0" name="pwordrecoverymaster[A1]" placeholder="Answer 1" value="<?php if($_POST && $errorCount > 0 ){echo $_POST['pwordrecoverymaster']['A1'];} ?>"/> 
											</div>
										    ...
										    <div class="container m-0 pl-2">
											    <input type="text" style="max-width:300px;" autocomplete="off" class="mt-1 form-control form-control-sm clearDefault mt-0" name="pwordrecoverymaster[A2]" placeholder="Answer 2" value="<?php if($_POST && $errorCount > 0 ){echo $_POST['pwordrecoverymaster']['A2'];} ?>"/> 
											</div>
					    				<!-- /Security Questions -->
					    	...
					    	<div class="container pt-2">
					    		<div class="row pt-3 justify-content-center">
					    			<div class=" col col-md-5 bg-white text-center pt-3 pb-5 signup-steps" style="max-height: 475px;overflow:scroll;">
					    				<h5 class="mb-2 text-gs">Step 3</h5>
					    				<div class="text-left text-black font-weight-bold">Church Info</div>

					    				<!-- Church Name -->
					    					<input type="text" class="form-control form-control-sm clearDefault mt-3" name="usermaster[sChurchName]" id="sChurchName" placeholder="Church Name" value="<?php if($_POST && $errorCount > 0 ){echo $_POST['usermaster']['sChurchName'];} ?>"/> 

					    				<!-- Contact Email Address -->
					    					<input type="email" class="form-control form-control-sm mt-4" name="usermaster[sContactEmailID]" id="sContactEmailID" placeholder="Church Email Address" value="<?php if($_POST && $errorCount > 0 ){echo $_POST['usermaster']['sContactEmailID'];} ?>">
										
					    				<!-- Church Denomination -->
							            	<select class="custom-select clearDefault mt-0 mt-4" name="usermaster[sDenomination]" id="denomination">
										...
										<!-- Church's Street Address -->
					    					<input type="text" class="form-control form-control-sm clearDefault mt-0 mt-4" name="usermaster[sAddress]" id="sAddress" placeholder="Church's Street Address" value="<?php if($_POST && $errorCount > 0 ){echo $_POST['usermaster']['sAddress'];} ?>"/> 
					    	...
					    	<div class="container pt-2">
					    		<div class="row pt-3 justify-content-center">
					    			<div class=" col col-md-5 bg-white text-center pt-3 pb-5 signup-steps" style="max-height: 475px;overflow:scroll;">
					    				...
					    				<div class="text-left text-black font-weight-bold mb-3">Church Info</div>
						    					...
						    					<div class="row">
						    						...
						    						<div class="col pl-md-5">
						    							<div class="custom-file mt-4">
														  	...
														  	<label class="custom-file-label" style=" text-align:left" for="sProfileName">Upload Pastor's Img</label>
														</div>
						    						</div>
						    					</div>
											</div>
										...
										<!-- Pastor's First and Last name -->
					    					<input type="text" class="form-control form-control-sm clearDefault mt-0 mt-4" name="usermaster[sPastorFirstName]" id="sPastorFirstName" placeholder="Pastor's First Name" value="<?php if($_POST && $errorCount > 0 ){echo $_POST['usermaster']['sPastorFirstName'];} ?>"/> 
					    					<input type="text" class="form-control form-control-sm clearDefault mt-0 mt-4" name="usermaster[sPastorLastName]" id="sPastorLastName" placeholder="Pastor's Last Name" value="<?php if($_POST && $errorCount > 0 ){echo $_POST['usermaster']['sPastorLastName'];} ?>"/> 
					    				<!-- /Pastor's First and Last name -->

					    				<!-- Pastor's Birth Date -->
										<div class="input-group date dateTime-input mt-4 mt-0" id="datetimepicker1">
						               		<input type="text" autocomplete="off" class="form-control form-control-sm clearDefault mt-0" id="DOB" name="usermaster[dDOB]" placeholder="Pastor's Brith Date" value="<?php if($_POST && $errorCount > 0 ){echo $_POST['usermaster']['dDOB'];} ?>"/> 
					    		...
					    		<div class="row pt-3 justify-content-center">
					    			<div class=" col col-md-5 bg-white text-center pt-3 pb-5 signup-steps">
										...
										<div class="container">
											...
											<input type="submit" id="btn_submit" class="btn btn-sm btn-primary" value="Create Profile"> <!-- d-none disabled -->
										</div>
				...
				if($('div.active').attr('id') == 'step5'){
					...
					/********************* Send Confirmation email ********************/
						...
						if(sendEmaiChecked){
							...
							sendMail.open("POST", "https://dev.gospelscout.com/emailConf.php");
							sendMail.send(formData); 

							/* Set the time out function within a function triggered by the submit button. */
			 					setTimeout(function(){ window.location.href = 'https://dev.gospelscout.com';},600000);
		 				}
			...
			*/
			...
			/* Verify confirmation code */
				$('#btn_checkCode').click(function(event){
					...
					checkConfCode.open('POST', "https://dev.gospelscout.com/emailConf.php");
