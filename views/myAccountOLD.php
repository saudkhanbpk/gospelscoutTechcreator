<?php 
	...
	/* Test For User Login */
		if($currentUserID == ''){
			...
			/* Get Usermaster data */
				$userData = $obj->fetchRow('usermaster', 'iLoginID = ' . $currentUserID);

			/* Get Log In Email */
				$loginData = $obj->fetchRow('loginmaster', 'iLoginID = ' . $currentUserID);
			...
			if($currentUserType == 'church'){
				...
				/* Get Amenity List */
					...
					$getAmenities = $obj->fetchRowAll('churchamenitymaster2', 'iLoginID=' . $currentUserID);
					
				/* Get Denominations */
					$getDenominations = $obj->fetchRowAll('denominationmaster', 'isActive=1');

				/* Get Service Times */
					$getServTimes = $obj->fetchRowAll('churchtimeing', 'iLoginID = ' . $currentUserID);
					// echo '<pre>';
					// var_dump($getServTimes);
			}
			...
			elseif($currentUserType == 'group'){
				$getMembers = $obj->fetchRowAll('groupmembersmaster', 'groupiLoginID = ' . $currentUserID);
				$getAllTalents = $obj->fetchRowAll('giftmaster', 'isActive = 1');
								...
								<form name="churchInfo" class="needs-validation" novalidate>
									...
									<div class="row pl-2">
										...
										<div class="col-7 col-md-3">
											<img class="artist-vid-img" src="<?php if( $userData['sProfileName'] ==''){echo '../img/gsStickerBig1.png';}else{echo URL . 'upload/church/' . $userData['sProfileName'];}?>" height="80px" alt="Card image cap" style="object-fit:cover; object-position:0,0;">
										</div>
									</div>
									<div class="row pl-2">
										<div class="col-10 col-md-7">
						...
						<form name="locationInfo" class="needs-validation" novalidate>
							...
							<div class="row pl-2">
								...
								<div class="col-10 col-md-7">
									...
									<input type="text" id="sCityName" class="form-control form-control-sm mb-2" name="sCityName" placeholder="City" value="<?php echo $userData['sCityName'];?>" required>
									<div class="invalid-feedback">
							          Please enter a city.
							        </div>
								</div>
							...
							<div class="row" id="zipStateErrorDiv" style="display:none">
								<div class="col pl-4 mt-2">
									<p class="text-danger" id="zipStateError"></p>
								</div>
							</div>
						</form>
						<input type="hidden" id="locParamCheck" val0="" value="">
						<hr class="mt-4"> <!-- Page Divider -->
					</div>
				<!-- /Location Form -->

				<!-- For Artists & Groups Only - Talent and bio Section -->
					<?php
												...
												<form name="talentRemove">
													...
													<div class="row pl-2">
								                        <div class="form-check py-2 col <?php if($currentUserType == 'artist'){echo 'col-md-4';}else{echo 'col-md-6';}?>" style="<?php if($currentUserType == 'artist'){echo 'max-height: 150px;overflow:scroll; font-size: 15px;border:1px solid rgba(0,0,0,.1)';}else{echo 'max-height: 250px;overflow:scroll';}?>" >
											  			...
											  			<div class="d-none">
												          Please select at least one talent.
												        </div>
													</div>
													<div class="row">
														<div class="col pl-4 mt-2">
											...
											<form name="delMin">
												...
												<div class="row pl-2">
							                        <div class="form-check border py-2 col col-md-4" style="max-height: 150px;overflow:scroll; font-size: 15px" >
											    		...
											    		?>
												    		<div class="ml-3"> <!--  -->
														    	 <input class="form-check-input ministries" name="removeChurchMin[]" type="checkbox"  value="<?php echo $min['iGiftID'];?>" id="ministryCheck<?php echo $i;?>">
																 <label class="form-check-label" for="ministryCheck<?php echo $i;?>">
										...
										<div class="container mt-3">
											<form name="addMin" class="needs-validation" novalidate>
												...
												<div class="row">
													...
													<div class='col co-md-5'>
														 <button type="submit" table="churchministrymaster2" class="btn btn-gs btn-sm mt-2" formId="addMin">Add Ministry</button>
													</div>
												</div>
											</form>
										</div>
									  ...
									  <div class="modal-dialog modal-dialog-centered" role="document">
									    <div class="modal-content">
										    ...
										    <!-- /Add New Photo Button -->
										      	...
										      	<div class="modal-footer">
										        	<button type="submit" class="btn btn-gs btn-sm sectionEdit" data-dismiss="modal" table="churchministrymaster2" formId="delMin">Delete Ministry</button>
										      	</div>
									    </div>
									  </div>
									</div>
	...
	/* Validate user's current password as part of a password update */
		function checkPword(){
			...
			if(currentPword != ''){
				...
				checkPwrd.open('POST','updateMyAccount');
	...
	/* Submit Form Changes for ministries and talents */	
		$('.sectionEdit').click(function(event){
			...
			/* Create New XMLHttpRequest */
				...
				updateSection.open('POST', 'updateMyAccount');
				updateSection.send(newFormD);
			/* END - Create New XMLHttpRequest */
		});

		/* Handle form validation and submit upon successful validation */
		(function() {
		  'use strict';
		      ...
		      form.addEventListener('submit', function(event) {
		        if (form.checkValidity() === false) {
		          ...
		          event.stopPropagation();
		     	  	...
		     	  	event.preventDefault();
		     	  	//console.log('test the for zip val');
							...
							if(updateSection.responseText != ''){
							    ...
							    /* Only fade out the successful update messages */	
							    	if(response === 'Your Account Has Been Updated!!!' || response === 'Your Password Has Been Updated!!!'){
								    	$("#acct-update-message").fadeOut(3000);
								    	 setTimeout(function(){ window.location.href = 'https://www.stage.gospelscout.com/views/myAccount.php';},3100);
								    }
							}
					}
				}
		...
		/* END - Handle form validation and submit upon successful validation */
			...
			if(userZip.length == 5){
				...
				var testJSON = $.getJSON('https://maps.googleapis.com/maps/api/geocode/json?address=' + userZip + '&key=AIzaSyCRtBdKK4NmLTuE8dsel7zlyq-iLbs6APQ&type=json&_=' + date.getTime(), function(response){
					...
					//find the city and state
						...
						/* Loop through the select options to grab the statename for comparison to api results */
