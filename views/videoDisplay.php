...
<div class="container px-0 px-md-5 viewAll-vid" style="margin-top:100px;">
	...
	<div class="container mx-0 px-0 px-md-2 pb-4 pt-3  viewAll-vid a-prof-shadow">
		<div class="container col-8 mt-0" style="max-width: 900px">
			<div class="row">
				<div class="col-sm-12  col-lg-8 mb-3 mb-lg-0 m-prof-sect" style="min-height:100px; ">
					...
					<!-- Display video comments -->
						<?php
							/* Query database for # of video Comments */
								$table = 'videocomments';
								$cond = 'videoID = ' . $vidId; 
								$commentCount = $obj->fetchNumOfRow($table,$cond,$db);
							...
							<div class="col  a-prof-shadow pt-2" style="min-height:100px;">
								...
								<hr class="my-3"><!-- Section Divider -->
									...
									<!-- the User comments will go here -->
										...
										<div id="showComments">
											<?php 
												foreach($allComments as $commentThread){?>
												<div class="row px-3" id="userComm_<?php echo $commentThread['iCommentID'];?>">
													<div class="media mb-3" style="min-width:98%; max-width:98%;">
													  	<img class="mr-3" src="<?php if($commentThread['sUserType'] == 'artist' || $commentThread['sUserType'] == 'group'){ echo $commentThread['sProfileName'];}elseif($commentThread['sUserType'] == 'church'){echo URL . 'upload/church/' . $commentThread['sProfileName'];}?> " width="40px" height="40px" alt="image"> <!-- echo URL . 'img/gsStickerBig1.png' -->
													    	...
													    	<p style="font-size:12px;" class="mb-0">
													   		...
													   		<?php
																/* Query database for # of Comment replies */
																	...
																	$replyCount = $obj->fetchNumOfRow($table1,$cond1,$db);
																/* END - Query database for video Comments */
															?>
															...
															<div class="container" style="width:90%;">
																...
																<div class="row d-none" id="<?php echo $commentThread['iCommentID'];?>">
																	<form method="post" class="" style="width:100%" enctype="multipart" action="" name="<?php echo $commentThread['iCommentID'];?>" id="addReplyForm<?php echo $commentThread['iCommentID'];?>">
																		...
																		<input type="hidden" name="sProfileName" value="<?php echo $userInfo['sProfileName'];?>">
																		<input type="hidden" name="sFirstName" value="<?php echo $userName;?>">
																		<input type="hidden" name="replyType" value="<?php echo $currentUserType;?>">
																		<div class="text-right">
				...
				<!-- Display Artists other videos of the same talent -->
					...
					<div class="mx-0 px-0 col-12 col-sm-7 col-lg-4 m-prof-sect" id="content-display" style="min-height: 100px;">
						<div class="container">
							<h6>more videos: </h6>
							<?php 
								$yu = 0;
							 ...
							 ?>	
							 		<!-- <a class="m-0 bg-info text-decoration-none" style="color:rgba(0,0,0,.65);" href="videoDisplay.php?vid_id=<?php echo $eachVidArray['id']; ?>&<?php if($_GET['artistID']){echo 'artistID=' . $artistUserID;}elseif($_GET['churchID']){echo 'churchID=' . $churchUserID;}?>&tal=<?php echo $eachVidArray['sGiftName'];?>">-->
							 		<a href="videoDisplay.php?vid_id=<?php echo $eachVidArray['id']; ?>&u_id=<?php echo trim( $_GET['u_id'] );?>&tal=<?php echo trim($_GET['tal']); ?>">
							 			<div class="row my-3">
							 				<div class="col-6">
												...
												?>
														<img class="card-img-top artist-vid-img" src="<?php if($eachVidArray['videoThumbnailPath'] ==''){echo '../img/gospelscout_logo.png';}else{echo $eachVidArray['videoThumbnailPath'];}?>" height="85px" width="50px" alt="Card image cap" style="object-fit:cover; object-position:0,0;">
												<?php
													}
												?>
											</div>
											<div class="col-6 px-0">
												...
												<!-- <ul class="list-unstyled card-text-size-gs  mb-0 ">-->
											    	<!-- <li class="text-truncate"><span class="d-inline-block"><?php if($eachVidArray['videoType'] == 'youtube'){echo 'Youtube Video';}else{echo $eachVidArray['videoName'];}?>;lj ljclv;jcljdflajdfladsjflasdjflksjkljlkjlj</span></li> -->
											    	<p class="m-0" style="font-size:.9em"><strong><?php if($eachVidArray['videoType'] == 'youtube'){echo 'Youtube Video';}else{echo truncateStr( $eachVidArray['videoName'].' this is the highlight clip for the nine nine and the two thousand', 50);}?></strong> </p>
											    	<!-- <li class="profCards-text-color"> -->
											    	<p class="m-0 text-muted" style="font-size:.8em"><?php echo truncateStr( $eachVidArray['sFirstName'] . ' ' . $eachVidArray['sLastName'], 20 );?></p>
											    	<p class="m-0 text-muted" style="font-size:.8em">
											    		<?php
...
<script src="<?php echo URL;?>views/testJS2.js"></script>
<script>
	/* Update Number of Views when page loads */
		var vidID = $('input[name=vidID]').val(); 
		var u_type = '<?php echo $_GET['churchID'];?>';

		if(u_type > 0){
			var view_type = 'c_views';
		}
		else{
			var view_type = 'views';
		}
		...
		viewsUpdate.onreadystatechange = function(){
			if(viewsUpdate.status == 200 && viewsUpdate.readyState == 4){
				console.log(viewsUpdate.responseText.trim() );
				document.getElementById('updateViews').innerHTML = viewsUpdate.responseText; 
			}
		}
		viewsUpdate.open('GET','vidDispDBupdate.php?vidID='+vidID+'&'+view_type+'=1');
		viewsUpdate.send(); 
		...
		$('#addLike').click(function(event){
			...
			addLike.onreadystatechange = function(){
				if(addLike.status == 200 && addLike.readyState == 4){
					console.log(addLike.responseText.trim() );
					...
					/* Create a data object to hold the comment reply data to be passed to the jS function */
						...
						while(xmlDoc.getElementsByTagName('reply')[i]){
							replyData[i] = { 
								...
								'usertype' : xmlDoc.getElementsByTagName('sUserType')[i].childNodes[0].nodeValue
							}
							i++; 
						}
	...
	/* Send Comment Replies to the Database */
		$('.addReply').click(function(event){
			...
			function dataObj(response){
				data['iCommentID'] = response; 
				data['usertype'] = '<?php echo $currentUserType;?>';
				return data; 
			}
