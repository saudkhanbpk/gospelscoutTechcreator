<?php 
	...
	/* Test For User Login */
		if($currentUserID == '' || $currentUserType != 'admin'){
			...
			/* Get Usermaster data */
				$userData = $obj->fetchRow('usermaster', 'iLoginID = ' . $user_id, $db);

			/* Get Log In Email */
				$loginData = $obj->fetchRow('loginmaster', 'iLoginID = ' . $user_id, $db);
			...
			if($user_type == 'church'){
				...
				/* Get the 2nd ministry table */
					$getAllMinistries2 = $obj->fetchRowAll('churchministrymaster', 'iLoginID=' . $user_id, $db);
					/*echo '<pre>';
					var_dump($getMin);
				...
				/* Get Amenity List */
					...
					$getAmenities = $obj->fetchRowAll('churchamenitymaster2', 'iLoginID=' . $user_id, $db);
					
				/* Get Denominations */
					$getDenominations = $obj->fetchRowAll('denominationmaster', 'isActive=1', $db);

				/* Get Service Times */
					$getServTimes = $obj->fetchRowAll('churchtimeing', 'iLoginID = ' . $user_id, $db);
			}
			elseif($user_type == 'artist'){

				/* Get the Talent List */  
					if($user_type == 'artist'){
						$getTalents = $obj->fetchRowAll('talentmaster', 'iLoginID = ' . $user_id, $db);
						$getAllTalents = $obj->fetchRowAll('giftmaster', 'isActive = 1', $db);
					}
			}
			elseif($user_type == 'group'){
				$getMembers = $obj->fetchRowAll('groupmembersmaster', 'groupiLoginID = ' . $user_id, $db);
				$getAllTalents = $obj->fetchRowAll('giftmaster', 'isActive = 1', $db);
...
?>
			...
			<div class="container bg-white mt-5 p-4" style="max-width: 900px;min-height:400px">
				...
				<div class="container mt-3">
					<h6><a class="text-gs font-weight-bold" href="<?php echo URL;?>adminDashboard/concept-master/manageusers-<?php if($user_type == 'artist' || $user_type == 'group'){echo 'artists';}else{echo $user_type;}?>-details.php?u_ID=<?php echo $user_id;?>"><- Admin Page</a></h6>
				</div>				
				<!-- Personal Info Form -->
								...
								<?php if($user_type == 'artist'){ ?>
									...
									<div class="row pl-2">
										...
										<div class="col-7 col-md-3">
											<img class="artist-vid-img" src="<?php if( $userData['sProfileName'] =='' || $userData['sProfileName'] == null){echo URL. 'img/dummy.jpg';}else{echo $userData['sProfileName'];}?>" height="80px" alt="Card image cap" style="object-fit:cover; object-position:0,0;">
										</div>
									</div>
								<?php }?>
							</div>
	...
	/* Validate user's current password as part of a password update */
		function checkPword(){
			...
			if(currentPword != ''){
				...
				checkPwrd.open('POST','<?php echo URL;?>views/updateMyAccount.php');
	...
	/* Submit Form Changes for ministries and talents */	
		$('.sectionEdit').click(function(event){
			...
			/* Create New XMLHttpRequest */
				...
				updateSection.open('POST', '<?php echo URL;?>views/updateMyAccount.php');
									...
									if(updateSection.responseText != ''){
									    ...
									    /* Only fade out the successful update messages */	
									    	if(response === 'Your Account Has Been Updated!!!' || response === 'Your Password Has Been Updated!!!'){
										    	$("#acct-update-message").fadeOut(3000);
										    	 setTimeout(function(){ window.location.href = 'https://www.gospelscout.com/adminDashboard/concept-master/adminedit-user-profile.php?u_type=<?php echo $user_type;?>&id=<?php echo $user_id;?>';},3100);
										    }
									}
							}
						}
						...
						/* check if location parameters are valid for sending request */
							...
							if(formName == 'locationInfo'){
								/* Insert the zip code validation and google query here */
									...
									validateZip_1(newZip,updateSection,newFormD);
							}
							else{
								updateSection.send(newFormD);
							}
...
/****************** validate Zip code *************************/
	function validateZip_1(userZip,updateSection,newFormD){
