 ...
 **************************************************************/
	...
	$('#artist_display_container').on('click','.addArtist', function(e){
		...
		var fname = `<a class="text-warning" target="_blank" href="https://www.stage.gospelscout.com/views/artistprofile.php?artist=${artist_requested_uid}">${artist_requested_fname}</a>`;
		...
		check_artInqiry_status_xhr.open('get', `https://www.stage.gospelscout.com/publicgigads/phpBackend/connectToDb.php?check_artist_status=${artist_requested_uid}&gigid=${$('input[name=gid]').val()}`);
    ...
    /* query db for requested artists */
          ...
          if(parsedResponse.length>0){
            for(let i in parsedResponse){
              if(parsedResponse[i].selected == "0" && parsedResponse[i].inquiry_withdrawn == "0"){
                var fname = `<a class="text-warning" target="_blank" href="https://www.stage.gospelscout.com/views/artistprofile.php?artist=${parsedResponse[i].iLoginID}">${parsedResponse[i].sFirstName}</a>`;
      ...
      art_req_xhr.open('get',  `https://www.stage.gospelscout.com/publicgigads/phpBackend/connectToDb.php?get_req_artists=true&gigid=${$('input[name=gid]').val()}`);
      art_req_xhr.send(); 
                    ...
                    if( new_gig_xhr.status == 200){
                      ...
                      if(parsedResponse.err_present){
                        /* Display error message */
                          ...
                          window.location = `https://www.stage.gospelscout.com/${page_redirect}?g_id=${gigID}`;
                      }
                      else{
                          /* Display unknown error message */
                            $('#zipError').html('Error: Unkown error - <a href="https://www.stage.gospelscout.com/contactUs/" class="text-gs">contact us</a>');
                      }
                      
                    }
                  }
                  new_gig_xhr.open('post', 'https://www.stage.gospelscout.com/publicgigads/phpBackend/connectToDb.php')
