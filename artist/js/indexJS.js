/* Search bar js */
	function hideIcon(self){
		if(self.value.length > 0){
			...
			self.style.backgroundImage = 'url(https://www.gospelscout.com/img/search_icon.png)';
		}
	}
...
/* Nav link functionality */
	$('.artist_search_method').click(function(event){
		...
		/* Clear the search form data */
			// document.forms.namedItem('form1').reset(); 
			// document.forms.namedItem('form2').reset(); 
			// document.forms.namedItem('form3').reset(); 
			// document.forms.namedItem('form4').reset(); 
			// document.forms.namedItem('form5').reset(); 
...
/* Call next subset of artists from pagination buttons */
	$('#pagination_container').on('click','.call_page',function(e){
		//e.preventDefault(); 
		...
		if(searchBy == 'form'){
			submitFormData(page_number);
		}
		else{
			if(searchBy == 'SBT'){
				searchBy[1] = $(this).attr('talent');
			}
			// call artist to page based on pagination number & search criteria
				getartistFunct(searchBy,'artist',page_number);
		}
		
	});

/* Handle artists requests from default menu */
	$('.searchBy').click(function(e){
		e.preventDefault(); 
		var searchBy = [$(this).attr('searchBy')];
		$('.searchBy').removeClass('active_categ');

		if(searchBy == 'SBT'){
			searchBy[1] = $(this).attr('talent');//console.log(searchBy);
			var tal_name = $(this).text(); console.log(tal_name);
			$('.tal_dropdown_pill').text(tal_name);
			$('.tal_dropdown_pill').addClass('active_categ');
		}else{
			$('.tal_dropdown_pill').removeClass('active_categ');
			$(this).addClass('active_categ');
		}

		/* Call get artist function */
			getartistFunct(searchBy,'artist',1);
	});

/* Handle Search criteria form */
	function submitFormData(page_no) {
...
/* Display artists modals */
	...
	$('#artist_display_container').on('click','.show_artist_deets', function(event){/*mouseenter*/
		...
		console.log(art_tal);
		console.log(art_id);
		var url = 'https://www.gospelscout.com/artist/phpBackend/connectToDb.php?art_talent='+art_tal+'&art_id='+art_id; 
	...
	/* Return Results for the Specified search criteria */
		$("#searchArtist").click(function(event){
		   ...
		   //          	transitionStyle: 'fade',
			...
			sendSearch.open("POST", 'https://www.gospelscout.com/artist/phpBackend/connectToDb.php');
...
/* Pagination */
	$('#show-artists').on('click','.page-item',function(event){
		...
		/* Fetch new set of artists */
			...
			get_artists_xhr.open('get','https://www.gospelscout.com/artist/phpBackend/connectToDb1.php?tab='+new_page+'&tal='+talent);
