<?php 
	...
	if(!empty($_POST)){
		...
		foreach($_POST as $index => $criteria){
			...
			if($criteria !== "" && $criteria !== "undefined"){
				...
				elseif($index == "sPastorFirstName" || $index == "sPastorLastName" || $index == "city") {
				...
				elseif($index == 'sAddress' || $index == "ChurchName"){
					$criteria = trim($criteria);
					$alphNumTest = str_replace(" ","",$criteria); 
					if(!ctype_alnum($alphNumTest)){
						if($index == "ChurchName"){
							echo '<div class="container text-gs pt-2" style="min-height: 200px"><h5>Please Enter A Valid Church Name (alpha-numeric characters only)!!!</h5></div>';
							exit;
						}
						else{
				...
				elseif($index == 'ministries'  || $index == 'amenities'){
	...
	if(count($newCriteriaArray) !== 0 && $newCriteriaArray !== null) {	
		...
		$churchSearchQuery .= "SELECT usermaster.iLoginID, usermaster.sDenomination,usermaster.sChurchName, usermaster.sPastorFirstName, usermaster.sProfileName,usermaster.sAddress, usermaster.iZipcode, usermaster.sCityName, states.statecode
							   FROM usermaster	
		...
		foreach($newCriteriaArray as $index => $criteria){
			...
			if($index == 'ministries' || $index == 'amenities'){
	...
	elseif($_GET['denom']){
		$denom = trim($_GET['denom']);
		$table = 'usermaster';
		$columnsArray = array('usermaster.iLoginID', 'usermaster.sChurchName', 'usermaster.sPastorFirstName', 'usermaster.sPastorLastName', 'usermaster.sProfileName', 'usermaster.sAddress', 'usermaster.sCityName', 'usermaster.sStateName', 'usermaster.iZipcode', 'usermaster.sDenomination', 'states.statecode');
		$paramArray['sDenomination']['='] = $denom;
		$paramArray['isActive']['='] = '1';
		$innerJoinArray = array(array('states','id', 'usermaster','sStateName'));
		$churchList = pdoQuery($table,$columnsArray,$paramArray,$orderByParam,$innerJoinArray);
		foreach($churchList as $cList){
			$churches[$denom][] = $cList;
		}
	}
	else{
		...
		/* Query denominationmaster table - Assign Artist's info to a card */
			...
			$churchQuery = "SELECT usermaster.iLoginID, usermaster.sChurchName, usermaster.sPastorFirstName, usermaster.sPastorLastName, usermaster.sProfileName, usermaster.sAddress, usermaster.sCityName, usermaster.sStateName, usermaster.iZipcode, usermaster.sDenomination, states.statecode
						    FROM usermaster 
						    INNER JOIN states on usermaster.sStateName = states.id 
						    WHERE  usermaster.sUserType = 'church' AND usermaster.isActive = 1 ";
			try{
				$churchResults = $db->prepare($churchQuery);
				$churchResults->execute(); 
				$churchList = $churchResults->fetchAll(PDO::FETCH_ASSOC); 

				foreach($churchList as $indexDenom => $valDenom){
					$currentDenom = $valDenom['sDenomination'];
					$churches[$currentDenom][] = $valDenom;
					    	...
					    	<div class="card-deck my-0">						    		
					    		...
					    		?>
							    	...
							    	<div class="card p-2 mb-0" style="width: 175px;margin:auto">
									  <img class="card-img-top d-block" style="object-fit:cover; object-position:0,0;" src="<?php echo $churchCard['sProfileName'];?>" width="100px" height="100px" alt="Card image cap">
									  <div class="card-body p-0 pt-0" style="min-height: 90px">
									  	<!-- Truncation Code for church names, pastor names, & addresses that are too long!!! -->
											<?php
												...
												$cityNameLength = strlen($churchCard['sCityName']);
										  		...
										  		if($cityNameLength > 10) {
										  			$churchCard['sCityName'] = substr_replace($churchCard['sCityName'], '...', 7); 
										  		}
									  		?>
									  	<!-- /Truncation Code for church names, pastor names, & addresses that are too long!!! -->
									    ...
									    <ul class="list-unstyled card-text-size-gs">
									    	...
									    	<li><?php echo $churchCard['sCityName'] . ', ' . $churchCard['statecode'];?></li>
									    </ul>
									  </div>
									  <!-- footer -->
									  <div class="card-footer p-0 pt-1 text-center mt-0">
									    <a href="<?php echo URL;?>views/churchprofile.php?church=<?php echo $churchCard['iLoginID'];?>" class="btn btn-primary btn-sm btn-gs">View Profile</a>
									  </div><!-- /footer -->
									</div><!-- /Card Info -->
								<?php 
									}
					    		...
					    		?>
					    			...
					    			<div class="col-4  px-2">
								    	<div class="card p-1 mb-0 card-shadow" style="height:249px">
										  <img class="card-img-top d-block" style="object-fit:cover; object-position:0,0;" src="<?php echo $churchCard['sProfileName'];?>" width="45px" height="110px" alt="Card image cap">
										  <div class="card-body p-0" style="min-height: 90px">
											...
											<?php
												...
												$cityNameLength = strlen($churchCard['sCityName']);
										  		...
										  		if($cityNameLength > 8) {
										  			$churchCard['sCityName'] = substr_replace($churchCard['sCityName'], '...', 5); 
										  		}
									  		?>
									  		<!-- /Truncation Code for church names, pastor names, & addresses that are too long!!! -->
					    		...
					    		?>
					    			<div class="col-3 px-4">
								    	...
								    	<!-- link to artist's profile pic -->
										  <img class="card-img-top d-block mb-0" style="object-fit:cover; object-position:0,0;" src="<?php echo $churchCard['sProfileName'];?>" width="45px" height="110px" alt="Card image cap">
										<!-- /link to artist's profile pic -->
										  <div class="card-body p-0" style="min-height: 90px">
										    ...
										    <ul class="list-unstyled card-text-size-gs">
										    	...
										    	<li><?php echo $churchCard['sAddress']; ?></li>
										    	<li><?php echo $churchCard['sCityName'] . ', ' . $churchCard['statecode'] . ' ' . $churchCard['iZipcode'];?></li>
										    </ul>
		 									
										  </div>
										  <div class="card-footer p-0 pt-1 text-center">
										    	<a href="<?php echo URL;?>views/churchprofile.php?church=<?php echo $churchCard['iLoginID'];?>" class="btn mb-0 btn-primary btn-sm btn-gs">View Profile</a><!-- link to the artist's profile -->
										  	</div>
										</div> 
									</div>
								<?php
