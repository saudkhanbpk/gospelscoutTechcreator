			...
			if($churchID > 0){
				...
				elseif($currentUserID > 0){		
					...
					/* Query the Database for artist's info */ 
						...
						$userRow = $obj->fetchRow("usermaster",$cond);
						if(!isset($userRow)){
							echo '<script>window.location = "'. URL .'newHomePage/search4churchNew.php";</script>';
							exit;
						}
				...
				echo '<script>window.location = "'. URL .'newHomePage/search4artistNew.php";</script>';
				exit;
			}
		}
		else{
		...
		<div class="smokeScreen m-0" style="height: 100%; width:100%">
			...
			<!--<div class="aProfPic-background">-->
				<div class="aProfPic-container">
					<img class="aProfPic" src="../upload/church/<?php echo $userRow['sProfileName'];?>" height="250px" width="250px">
				</div>
			<!--</div>-->
		</div>
				...
				<!-- Display Upcoming Events -->
					...
					<div class="bg-white mt-3 p-2 a-prof-shadow" id="Events" style="min-height:250px">
						...
						<div class="mt-5 p-0 pt-1 text-center">
					    	<a href="<?php echo URL;?>newHomePage/calendar/calendarDisplay.php?u_Id=<?php echo $artistUserID;?>" class="btn mb-0 btn-primary btn-sm btn-gs">View Full Event Calendar</a>
					  	</div>
					</div> 
				<!-- /Display Upcoming Events -->
			</div>
							...
							<!-- DISPLAY CONTENT RETURNED FROM THE XMLHTTPREQUESTS -->
								<div class="container" id="contentContainer">
									<div class="tab-content Vid" id="">
										<?php include(realpath($_SERVER['DOCUMENT_ROOT']) . '/views/xmlhttprequest/churchProfVidTab.php'); ?>
									</div>
	...
	/* Add a New Video */
		...
		form1.addEventListener('submit', function(event){
			...
			/* Prevent Users from embedding youtube links and uploading video simaltaneously */
				var vidMethod = '';
				if($('#youtubeInput').val() != '' && ($('#vidName').val() != '' || $('#vidDescr').val() != '' || $('#videoFile').val() != '' || $('#thumbnailFile').val() != '')) {
			...
			/* If video method is assigned, send form info to fileUpload.php */
				if(vidMethod != ''){
					...
					*/
					...
					addNewVid.open("POST", "churchFileUpload.php"); 
					/*
