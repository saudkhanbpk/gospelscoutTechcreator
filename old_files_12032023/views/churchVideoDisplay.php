<?php 
	...
	elseif($_GET['churchID']){
		/* Query Database for church Video info based on the talent passed in $_GET['min'] */
			...
			$result = $vidArray->fetchAll(PDO::FETCH_ASSOC);
		/* END - Query Database for church Video info based on the talent passed in $_GET['min'] */
...
<!-- Video.js javascript and css files -->
	<head>
	    ...
	    <script src="<?php echo URL;?>newHomePage/node_modules/video.js/dist/video.js"></script>
	    
	</head>
<!-- /Video.js javascript and css files -->

<div class="container mt-4 px-5 viewAll-vid">
	<a href="<?php echo URL . 'newHomePage/views/churchprofile.php?church=' . $churchUserID; ?>"class="text-gs"><h4 class="text-capitalize text-gs text-center text-lg-left"><?php echo $result[0]['sChurchName']; ?></h4></a>
					...
					<!-- Display the video being watched -->
						<div class="row" id="videoDisplay">
							<div class="col a-prof-shadow p-2 embed-responsive embed-responsive-16by9" style="min-height:100px;"> 
								...
								?>
									  	<iframe  class="embed-responsive-item" src="<?php echo $youtubeLink; ?>" frameborder="0" allowfullscreen></iframe>
								<?php
									  }
								?>
							</div>
							...
							<div class="col  a-prof-shadow pt-2" style="min-height:100px;">
								...
								<?php if($currentUserID){?>
									...
									<form method="post" enctype="multipart" action="" name="addComment" id="addCommentForm">
										...
										<input type="hidden" name="c_comment" value="1">
									...
									<!-- the User comments will go here -->
										<?php 
											/* Query database for video Comments */
												$commentQuery = 'SELECT churchvideocomments.*, usermaster.sProfileName, usermaster.sFirstName, usermaster.sLastName, usermaster.sChurchName
																 FROM churchvideocomments
																 INNER JOIN usermaster on churchvideocomments.iLoginID = usermaster.iLoginID
																 WHERE churchvideocomments.videoID = ?
											...
											<?php foreach($allComments as $commentThread){ ?>
												<div class="row px-3" id="userComm_<?php echo $commentThread['iCommentID'];?>">
													<div class="media mb-3" style="min-width:98%; max-width:98%;">
													  	<img class="mr-3" src="<?php echo URL;?>newHomePage/upload/artist/<?php echo $commentThread['sProfileName'];?> " width="40px" height="40px" alt="image">
													  	<div class="media-body " >
													  		<div class="container m-0">
													  			<div class="row">
													  				<div class="col-6 p-0">
																    	<h6 class="mt-0"><?php  if($commentThread['sChurchName']){ echo $commentThread['sChurchName'];}else{echo $commentThread['sFirstName'];}?>  
													    	...
													    	<p style="font-size:12px;" class="mb-0">
													   		...
													   		<?php
																/* Query database for # of Comment replies */
																	$table1 = 'churchvideocommentreplies';
															...
															<div class="container" style="width:90%;">
																...
																<div class="row d-none" id="<?php echo $commentThread['iCommentID'];?>">
																	<form method="post" class="" style="width:100%" enctype="multipart" action="" name="<?php echo $commentThread['iCommentID'];?>" id="addReplyForm<?php echo $commentThread['iCommentID'];?>">
																		...
																		<input type="hidden" name="sProfileName" value="<?php echo $userInfo['sProfileName'];?>">
																		<input type="hidden" name="sFirstName" value="<?php echo $userName;?>">
																		<input type="hidden" name="replyType" value="church">
																		<div class="text-right">
									 ...
									 ?>	
										<!-- Card Content -->
											<div class="card col-12 p-md-2 border-light mx-auto mx-md-0">
												<a class="m-0 bg-info" href="videoDisplay.php?vid_id=<?php echo $eachVidArray['id']; ?>&artistID=<?php echo $artistUserID;?>&tal=<?php echo $eachVidArray['sGiftName'];?>">
	    	...
	    	<div class="container p-3 text-center mb-0 d-none" id="error-message"><p class="m-0 p-2 card-shadow text-white bg-gs" id="error-text" style="border-radius:7px"></p></div>
	      	...
	      	<div id="reportStatus">
		      	<form action="" method="POST" name="reportVideo" id="reportVideo" enctype="multipart/form-data">
			      		...
			      		<?php 
			      			/* Reasons for Reporting a Video */
			      				$reportReasons = array(
			      										...
			      										'Video Content Does Not Belong To Church',
			      										'Other:'
			      									);
			      				$n = 0;
			     	...
			     	<input type="hidden" name="churchId" value="<?php echo $_GET['churchID'];?>">
...
<script src="<?php echo URL;?>newHomePage/views/testJS.js"></script>
<script>
	/* Update Number of Views when page loads */
		...
		viewsUpdate.open('GET', 'vidDispDBupdate.php?vidID='+vidID+'&c_views=1');
		viewsUpdate.send(); 
	...
	/* Like-button functionaility */
		$('#addLike').click(function(event){
			...
			addLike.open('GET', 'vidDispDBupdate.php?vidID='+vidID+'&c_likes=1');
			addLike.send(); 
		});
	...
	/* Report Video For Innappropriate Content */
		...
		reportForm.addEventListener('submit', function(event){
			...
			reportVidForm.append('c_report', 1);
	...
	/* Get Video Comment replies */
		$('.getReplies').click(function(event){
			...
			getReplies.open('GET', 'vidDispDBupdate.php?getComments=1&c_repliesTo='+commId+'&vidId='+vidId);
			getReplies.send();
		});
	/* END - Get Video Comments replies */
	...
	/* Remove Comment */
		$('.removeComment').click(function(event){
		 	...
		 	removeComment.open('GET', 'vidDispDBupdate.php?deleteComment=church&iCommentID='+comID);
		 	removeComment.send(); 
		 });
	/* END - Remove Comment */
</script>