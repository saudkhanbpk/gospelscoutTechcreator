<?php 
	...
	/* Remove all non alpha-numeric characters */
		$vidName = preg_replace("/[^A-Za-z0-9 ]/", '', $postArray['videoName']);
					...
					/* PROCESS UPLOADED VIDEO CONTENT */
						/**** Create correct directory corresponding to church vs artist ****/
							if($postArray['u_type'] == 'artist' || $postArray['u_type'] == 'group'){
								$vidDir = '/upload/video/artist/artist_' . $postArray['iLoginID'] . '/video/'; 
								$thumbDir = '/upload/video/artist/artist_' . $postArray['iLoginID'] . '/thumbnail/';
							}
							elseif($postArray['u_type'] == 'church'){
								$vidDir = '/upload/video/church/church_' . $postArray['iLoginID'] . '/video/'; 
								$thumbDir = '/upload/video/church/church_' . $postArray['iLoginID'] . '/thumbnail/';
							}
							...
							if($newFileType == "videoFile"){
								...
								if(count($newIndivFile) > 0){
									...
									elseif($viddioFileSize != '' && $viddioFileSize > 120000000) {
										...
										$target_file_vid = $target_dir . $randNumb . str_replace(' ', '', $vidName) . '.' . $vidioFileType;
								...
								if(count($newIndivFile) > 0){
									...
									elseif($viddioFileSize != '' && $viddioFileSize > 1200000) {
										...
										$target_file_thumb = $target_dir_thumb . $randNumb. str_replace(" ","",$vidName) . '.' . $vidioFileType; 
					...
					/* If admin deleted this video update the table row otherwise delete table row */
						if($postArray['adminID'] > 0){
							...
							foreach($postArray as $adminDelIndex => $adminDel){
								$field_admnDel[] = $adminDelIndex; 
								$value_admnDel[] = $adminDel; 
							}
							/*echo '<pre>';
			var_dump($postArray);
			exit;*/
							$wasDeleted = $obj->update($field_admnDel,$value_admnDel,$cond,$table);
						}
						else{
							$wasDeleted = $obj->delete($table,$cond);
						}
					/* If video was successfully removed, remove the associated comments and replies */	
						if($wasDeleted){
							$commentsDeleted = $obj->delete($table1,$cond1);
							$repliesDeleted = $obj->delete($table2,$cond1);
						}
