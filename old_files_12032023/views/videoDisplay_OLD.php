...
<div class="container mt-4 px-5 viewAll-vid">
	<a href="<?php echo URL . 'views/artistprofile.php?artist=' . $artistUserID; ?>"class="text-gs"><h4 class="text-capitalize text-gs text-center text-lg-left"><?php if($_GET['tal'] == 'group'){echo $result[0]['sGroupName'];}else{  echo $result[0]['sFirstName'] . ' ' . $result[0]['sLastName'];} ?></h4></a>
											...
											<?php foreach($allComments as $commentThread){ ?>
												<div class="row px-3" id="userComm_<?php echo $commentThread['iCommentID'];?>">
													<div class="media mb-3" style="min-width:98%; max-width:98%;">
													  	<img class="mr-3 test" src="<?php echo URL;?>upload/artist/<?php echo $commentThread['sProfileName'];?> " width="40px" height="40px" alt="image">
...
<script src="<?php echo URL;?>views/testJS.js?v=1.2"></script>
<script>
	...
	/* Report Video For Innappropriate Content */
		...
		reportForm.addEventListener('submit', function(event){
			...
			badVid.open('POST', 'vidDispDBupdate');
		...
		$('.getReplies').click(function(event){
			...
			getReplies.onreadystatechange = function(){
				if(getReplies.status ==200 && getReplies.readyState == 4){
					...
					//console.log('response text says: '+getReplies.responseText);
