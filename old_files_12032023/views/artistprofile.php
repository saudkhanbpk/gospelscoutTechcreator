<?php 
	...
	/* Check for the user classification - define corresponding indicator */
		if(isset($_GET['artist'])) {
			...
			if($artistID > 0){
				...
				/* Query the Database for artist's info */ 
					...
					if(!isset($userRow) || $userRow[0]['sUserType'] == 'gen_user'){
			...
			if($currentUserID > 0 && ($currentUserType == 'artist' || $currentUserType == 'group')){
				...
				echo '<input type="hidden" name="userID" class="userID" value="' . $currentUserID . '" id="1">';
				...
				/* Query the Database for artist's info */ 
					...
					if($userRow[0]['sUserType'] == 'gen_user'){
									...
									/* Update db with user's account ID */
										if($upateResponse->settings->payouts->schedule->interval == 'manual'){
											/* Update Usermaster with the Connected acct id*/
												$table100 = 'usermaster';
												$array100['str_connectActID'] = $connected_account_id;
												$cond100 = 'iLoginID = ' . $currentUserID;
												$upD_cust = updateTable($db, $array, $cond100, $table100);
		...
		/* Check if user profile image is null or empyt */
			if($userRow[0]['sProfileName'] == null || $userRow[0]['sProfileName'] == ''){
				$userRow[0]['sProfileName'] = URL . 'img/dummy.jpg';
			}
					...
					<div class="bg-white p-3 a-prof-shadow" id="artistInfo">
						...
						<div id="infoTable">
								...
								<div class="row my-2 text-gs" style="font-size: 13px">
									...
									<div class="col" title="<?php echo $sContactEmailID;?>"> <!--text-truncate-->
										<a class="text-gs" href="#" data-toggle="popover" data-placement="top" title="Artist's Email:" data-content="<?php echo $userRow[0]['sContactEmailID'];?>">
                                            <?php
                                                $email_truncated = truncateStr($userRow[0]['sContactEmailID'], 15);
                                                echo $email_truncated; 
                                                // echo $userRow[0]['sContactEmailID']; 
                                            ?>
                                        </a>
									</div>
								</div>		
				...
				<!-- Display Upcoming Events -->
					...
					<div class="bg-white mt-3 p-2 a-prof-shadow" id="Events" style="min-height:250px">
						...
						<!-- Loop needs to be created to output the latest 4 events and/or gigs -->
						 <div class="row">
							<div class="col-11" id="upcEvents" style="min-height: 90px; margin:auto">
						...
						<div class="mt-2 p-0 pt-1 text-center">
					    	<a href="#" id="viewCal" class="btn mb-0 btn-primary btn-sm btn-gs">View Full Event Calendar</a><!--"<?php echo URL;?>calendar/?u_Id=<?php echo $artistUserID;?>"-->
					  	</div>
					</div> 
				<!-- /Display Upcoming Events -->
			</div>
...
<script src="<?php echo URL;?>js/artistprofileJSFunctions.js?104"></script>
<script src="<?php echo URL;?>js/artistprofileJS.js?60"></script>			