...
if(isset($_GET['type'])){
	...
	if($_GET['type'] == 'user'){
		$cond=" isActive IN(0) AND sUserType = 'user' order by iUserID DESC";	
	}
	
	if($_GET['type'] == 'artist'){
		$cond=" isActive IN(0) AND sUserType = 'artist' order by iUserID DESC";	
	}
	
	if($_GET['type'] == 'church'){
		$cond=" isActive IN(0) AND sUserType = 'church' order by iUserID DESC";	
	}
	...
	$cond=" isActive IN(0) order by iUserID DESC";
	$usermaster = $obj->fetchRowAll('usermaster',$cond);
	$totalusermaster = $obj->fetchNumOfRow('usermaster',$cond);	
}
...
function searchfilter()
{
		...
		xmlhttp.open("GET","<?php echo HTTP_SERVER;?>views/loaduser1.php?mixvalue="+mixvalue+"&usertype="+usertype,true);
		xmlhttp.send();
	}
</script>
<div id="page-wrapper">
...
//					$catRow = $obj->fetchRow('categorymaster',$cond);	
            ?>
              <tr>
				...
				<?php
				$cond1=" isActive IN(0) AND iLoginID = '".$usermaster[$i]['iLoginID']."'";
                ...
                <td><a href="<?php echo HTTP_SERVER;?>views/userdetails.php?iLoginID=<?php echo $usermaster[$i]['iLoginID'];?>" title="View"><i class="fa fa-fw fa-eye"></i></a> | 
                    <a href="<?php echo HTTP_SERVER;?>views/viewdeactiveusers.php?iLoginID=<?php echo $usermaster[$i]['iLoginID'];?>" title="Active" ><i class="fa fa-fw fa-check"></i></a></td>
              </tr>  
            <?php } ?>            
            </tbody>
          </table>
...
if(isset($_GET['iLoginID'])){
		...
		if($obj->update($field, $value, "iLoginID = ".$_GET['iLoginID'],'usermaster') == true){
			...
			$objsession->set('gs_message','User successfully Activated.');	
			redirect(HTTP_SERVER."views/viewdeactiveusers");
		}
