...
$eventmaster = $obj->fetchRowAll('bookingmaster',$cond);
$totaleventmaster = $obj->fetchNumOfRow('bookingmaster',$cond);
?>
		...
		$eventmaster3 = $obj->fetchRow('eventtypes',$cond3);  
            ...
            ?>
              ...
              <td><?php if($eventmaster2 != ''){echo $eventmaster2['sFirstName']." ".$eventmaster2['sLastName'];}else{echo '---';}?></td>
                <td><a href="<?php echo HTTP_SERVER;?>views/artistbook.php?bookIDS=<?php echo $eventmaster[$i]['iBookingID'];?>&satus=<?php  echo $eventmaster[$i]['sStatus']; ?>"><?php echo $eventmaster[$i]['sStatus'];?></a></td>              
                <td>
                    <a href="<?php echo HTTP_SERVER;?>views/artistbook.php?bookID=<?php echo $eventmaster[$i]['iBookingID'];?>" title="Delete" onClick="return confirm('Are you sure want to delete?');"><i class="fa fa-fw fa-remove"></i></a></td>
              </tr>  
            <?php } ?>            
            </tbody>
          </table>
...
if($_GET['satus']=="Pending"){
		...
		if($obj->update($field, $value, "iBookingID = ".$_GET['bookIDS'],'bookingmaster') == true){
