 ...
 ***********************************************/
 $('.getGigTalInqu').click(function(event){
    ...
    /* Get all artist inquiries on page load */
        ...
        get_tal_inquiries.open('get','https://www.gospelscout.com/publicgigads/phpBackend/connectToDb_ad_details.php?get_all_inqu='+gigId+'&gmRequested='+gmRequested+'&tal_type='+artistType+'&tal_track='+talTrackerId);
        ...
        canc_artist_xhr.onload = () => {
            if(canc_artist_xhr.status == 200){
                ...
                if(parsedResponse.artist_canc){
                    ...
                    $('#canc-conf-mess').html('<p class="text-warning">Cancellation was Un-Successful.  <a href="https://www.gospelscout.com/home/#Contact-Us">Please contact us!</a></p>');
                    $('#art_req_canc_buttons').html('<button class="btn btn-sm btn-gs" data-dismiss="modal" type="button">close</button>');
                }
            }
        }
        canc_artist_xhr.open('post','https://www.gospelscout.com/publicgigads/phpBackend/connectToDb_ad_details.php');
