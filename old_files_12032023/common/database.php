...
class Database {
	...
	function __construct(){
		...
		if(define('HOST',"localhost:3306")){
			$this->_hostname = HOST;	
		}
		if(define('USER','u7fnwik4zjott')){
			$this->_username = USER;	
		}
		if(define('PASSWORD',"nehs4jr9gfcn")){
			$this->_password = PASSWORD;
		}
		if(define('DATABASE',"dbipzydpgopuqj")){
			 $this->_database = DATABASE;
		}
		
		// $connect = @mysql_connect($this->_hostname,$this->_username,$this->_password);
		// $connect = @mysql_connect($this->_hostname,$this->_username,$this->_password, $this->_database);
    	
		// if(!$connect){
		// 	die('Unable to connect in MySqL Server.');
		// }
	...
	public function fetchRowAll($table,$cond, $db){
		...
		try{
			// if($table != ""){
				$this->_table = $table;	
				 ...
				 /* PDO Delete Statement */
					...
					$fetchData = $db->query($query);
					// return $fetchData;
					$fetchData->execute(); 
					$queryDBResults = $fetchData->fetchAll(PDO::FETCH_ASSOC);
			...
			// // Create connection
			// $conn = new mysqli($this->_hostname,$this->_username,$this->_password, $this->_database);
			// $sql = "SELECT * FROM `".$this->_database."`.`".$this->getTable()."` WHERE ".$cond; 
			// // Check connection
			...
			// return 'test:'.$result;
			// var_dump($connect);
			// return $this->_hostname.' '.$this->_username.' '.$this->_password.' '. $this->_database;
	...
	public function fetchNumOfRow($table,$cond,$db){
	...
	public function fetchRow($table,$cond,$db){
