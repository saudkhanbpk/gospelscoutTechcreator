<?php 
	...
	function sendContactPageEmail($userName, $userID, $userEmail, $userInitialMess, $userMessTimestamp, $adminName, $adminID, $adminReply, $messageID) {
	  // Create Client "contact_pagereceive@gospelscout.com"; , $city, $state, $mobile "commenter_mobile" => $mobile,  "city" => $city, "state" => $state, 
	  $client = new PostmarkClient("5e5025bd-5c9b-4c2b-899c-772d825f85f3");
	  ...
	  $receiverName = "Contact Page - Admin";
		...
		try{
			...
			$sendResult = $client->sendEmailWithTemplate(
				...
				"message_timestamp" => $userMessTimestamp,
				"body" => $userInitialMess,
				]
			);
	...
	class popupworship {
		...
		/* Notify popupworship applicants they've been selected to attend */
			...
			$sendResult = $client->sendEmailWithTemplate(
				$sender,
		      	...
		      	[
					...
					"action_url"=>'https://dev.gospelscout.com/popupworship/attend_conf?id='.$recipient_array['id'],
					"support_email"=>$recipient_array['fName'],
					"help_url"=>$sender,
					"product_url"=>'httsp://dev.gospelscout.com/popupworship/',
		...
		/* Notify popupworship applicants of the event location */
		function sendPuwLocation($recipient_array,$puwSection){
			...
			try{
				...
				$response['error']['httpStatusCode'] = $e->httpStatusCode;
				$response['error']['postmarkApiErrorCode'] = $e->postmarkApiErrorCode;
		...
		/* Send batch PUW artist request emails */
			function sendBatchPuwArtistRequest($recipients,$gen_info){
				$client = new PostmarkClient("5e5025bd-5c9b-4c2b-899c-772d825f85f3");
			    $sender = "administrator@gospelscout.com"; 
			    $TemplateID = 16382329;
			   
			    foreach($recipients as $recipients_index => $recipients_val){
			    	$recipient_array = array_merge($recipients_val,$gen_info);
			    	 $message_array[] = ['To' => $recipient_array['sContactEmailID'],
							 'From' => $sender,
				             'TemplateID' =>$TemplateID,
				             'TemplateModel'=>[
				             	"name" =>$recipient_array['sFirstName'].' '.$recipient_array['sLastName'],
								"product_name"=>$recipient_array['puwSection'],
								"event_date"=>$recipient_array['date'],
								"event_start_time"=>$recipient_array['startTime'],
								"event_end_time"=>$recipient_array['endTime'],
								"event_address"=> $recipient_array['address'],
								"event_city_state_zip"=> $recipient_array['city'].', '.$recipient_array['state'].' '.$recipient_array['zip'],
								"talent"=>$recipient_array['sGiftName'],
								"action_url"=>$recipient_array['action_url'],
								"support_email"=>$recipient_array['fName'],
								"help_url"=>$sender,
								"product_url"=>'httsp://dev.gospelscout.com/popupworship/',
								"company_name"=>'GospelScout',
	            ...
	            try{
	            	$resp = $client->sendEmailBatch($message_array);
	            	$response['resp_obj'] = $resp;
	            	$i=0;
	            	foreach($resp as $response_ind => $response_val){
	            		if($response_val->ErrorCode !== 0 || $response_val->Message !== 'OK'){
	            			/* There was an error */
		            			$response['send_error']['present'] = $i + 1;
		            			$response['send_error']['message'][] = $response_val->Message;
	            		}
	            		else{
	            			$response['send_error']['present'] = false;
	            		}
	            	}
	            }catch(PostmarkException $p_e){
	            	$response['error']['present'] = true;
					$response['error']['httpStatusCode'] = $p_e->httpStatusCode;
					$response['error']['postmarkApiErrorCode'] = $p_e->postmarkApiErrorCode;
					$response['error']['message'] = $p_e->message;
				}catch(Exception $e){
					$response['error']['present'] = true;
					$response['error']['message'] = $e->message;
				}
	...
	class booking {
		...
		function request($requestor, $requestorName, $requestorUrl, $receiver, $receiverName, $talentRequested, $actionUrl, $gigName, $gigId, $gigPay){
			//Create Client
		    $client = new PostmarkClient("5e5025bd-5c9b-4c2b-899c-772d825f85f3");
		    $sender = "administrator@gospelscout.com"; 
			$emailID = 3581901;
		...
		function update($requestor, $requestorName, $requestorUrl, $receiver, $receiverName, $talentRequested, $actionUrl, $gigName, $gigId, $email_editInfoParam) {
			...
			$emailID = 3593443;
			$date = date("F j, Y, g:i a");
			//"kirkddrummond@yahoo.com";
			//Identify updated sections & store in var as a comma seperated list
			$indivParam = "";
