 	...
 	$(document).ready(function(){
 		...
 		*/
 		...
 		$('#calendar').fullCalendar({
 			...
 			footer: {
 				center: 'month,agendaDay,listYear'
 			},
 			buttonIcons: false,
 			fixedWeekCount: false,
 			//weekNumbers: true, agendaWeek,
 			eventLimit: true, 
 			views: {
 				month: {
 			...
 			*/
 			/* Event Settings */
 			events: 'myFeed.php?u_Id='+u_Id+'&status='+status, //Need to pass a user id and if a user is logged in and viewing their own calendar, a session id; session id will enable the myfeed.php to return private gigs/events
...
<div class="container py-md-3 px-md-5 mt-3 mt-md-0">
	...
	<?php }
		  	...
		  	if($userRow['sUserType'] == 'artist'){
		  		$displayName = $userRow['sFirstName'];
		  	}
		  	elseif($userRow['sUserType'] == 'group'){
		  		$displayName = $userRow['sGroupName'];
		  	}
		  	elseif($userRow['sUserType'] == 'church'){
		  		$displayName = $userRow['sChurchName'];
		  	}
	?>
				<div class="container" style="max-width: 860px;">
					<h5 class="text-capitalize text-gs text-center text-lg-left"><?php echo $displayName; ?>'s Calendar</h5>
				</div>
	<?php } ?>
