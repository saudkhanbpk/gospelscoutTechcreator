...
/* send confirmation email */
	function sendConfCode(){
		...
		if(email_is_present != ''){
			...
			// sendMail.open("POST", "https://www.gospelscout.com/signup/artist/phpBackend/emailConf.php");
			sendMail.open('POST', 'https://www.gospelscout.com/views/checklogin.php');
    ...
    /* Set validation rules & corresponding error-messages */
      rules:{
        ...
        'loginmaster[sEmailID]': {
              ...
              email: true,
              remote:({
                url: 'https://www.gospelscout.com/signup/artist/phpBackend/connectToDb.php',
                //  success: (resp) => {
                // 	 console.log(resp);
                //  } 
              })
...
function validateZip1(uState,zipCode) {
        ...
        if(userZip.length == 5){
            ...
            $.getJSON('https://maps.googleapis.com/maps/api/geocode/json?address=' + userZip + '&key=AIzaSyCRtBdKK4NmLTuE8dsel7zlyq-iLbs6APQ&type=json&_=' + date.getTime(), function(response){
                //find the city and state
                    ...
                    if(state == userState){
                      console.log(`states match=> ${state} = ${userState}`);
					  console.log(`states match=> ${state} = ${userState}`);
                        $('#sCityName').val(city);
                        $('#apiStateName').val(state);
                    }
                    else{
                      console.log(`states dont match=> ${state} != ${userState}`);
                        $('#sCityName').val('');
                    }
            });
        }
...
var test =  $("#form-total").steps({
       ...
       **/
          /* define current inputs depending on current step */
              if( currentIndex == 0 ){
                  ...
                  if(uState != '' && zipCode!= ''){
                     //Note: validateZip1 is not being used!!! 
					  
                    let userCity = new validateZip(zipCode, uState);             
                    userCity.getCityStateOBj
                    .then(userCity.returnCity)
                    .then(cityFound => {
                       	$('#sCityName').val(cityFound.city);
                    	$('#apiStateName').val(cityFound.state);
                    });
                  }
                  ...
                  var currentInputs = $('section.sect-3 input,section.sect-3 select');
              }
  ...
  onFinishing: function (event, currentIndex) { 
    /* handle form submission - AJAX Request */
			...
			//   console.log(response);
              ...
              if(parsedResponse.errorCount > 0){
                  if(parsedResponse.errorMessage === 'conf_code_invalid'){
                    ...
                    $('#error-display').html(parsedResponse.errorMessage);
                  }
        ...
        xhr.open('post','https://www.gospelscout.com/signup/artist/phpBackend/connectToDb.php?l');
        xhr.send(formOBJ);
      },
  })
