...
/******************************************* Query Database with PDO Method *************************************/
	...
	*/
	...
	function pdoQuery($table,$columnsArray,$paramArray,$orderByParam,$innerJoinArray,$leftJoinArray){
					...
					foreach($value1 as $index1_5 => $value1_5){
						if($j == $loopMax1){
							if($index1 == 'postedgigsmaster.MONTH(gigDate)'){
								$cond .= 'MONTH(gigDate)'. $index1_5 . ' :getMonth';//$index1 .
							}
							else{
							...
							if($index1 == 'postedgigsmaster.MONTH(gigDate)'){
								$cond .= 'MONTH(gigDate)'. $index1_5 . ' :getMonth AND ';//$index1 .
							}
							else{
...
/************************************************************** Notification function **************************************************************/
	function createNotification($db, $obj, $action, $notifier, $notified, $link){
		...
		/********************* Define a switch statement to determine the link path *********************/
			switch ($action) {
              case 'gigRequest':
                $insertArray['link'] = URL . 'newHomePage/views/gigform.php?gigID=' . $link;
                break;

              case 'gigInquiry':
                $insertArray['link'] = URL . 'newHomePage/publicgigads/ad_details.php?g_id=' . $link;
                break;

              case 'suggestGig':
                $insertArray['link'] = URL . 'newHomePage/publicgigads/ad_details.php?g_id=' . $link; //4472756d343938?id=
                break;
              ...
              case 'vidNotification':
                $insertArray['link'] = URL . 'newHomePage/views/';
                break;
	...
	function findArtistMatch($db, $obj, $gigInfo){
	    ...
	    /* Create query statement and query database */ 
	        ...
	        /* Select appropriate column corresponding to the userType */
	            if($gigInfo['userType'] == 'artist'){
	            	$findMatchQuery .= '(';
	            	$tal_count = count( $gigInfo['art_needed'] );
	            	$counter = 1;

	            	foreach($gigInfo['art_needed'] as $tal_needed){
	            		if($counter < $tal_count){
	            			$findMatchQuery .= 'talentmaster.TalentID = :TalentID_'.$tal_needed['artistType'] . ' OR ';
	            		}
	            		else{
	            			$findMatchQuery .= 'talentmaster.TalentID = :TalentID_'.$tal_needed['artistType'] . ')';
	            		}
	        ...
	        try{
	            ...
	            /* Select appropriate bindParam corresponding to the userType */
		            if($gigInfo['userType'] == 'artist'){
		            	...
		            	foreach($gigInfo['art_needed'] as $tal_needed1){
		                	$findMatch->bindParam(':TalentID_'.$tal_needed1['artistType'],$tal_needed1['artistType']);
		                }
		            }
		            elseif($gigInfo['userType'] == 'group'){
	...
	function handlefileUpload($userType, $fileType, $iLoginID) {
		...
		/* PROCESS UPLOADED Profile IMG */
			switch ($fileType){
				...
				case "profileImg":
					$Dir = '/newHomePage/upload/'. $userType . '/'.$iLoginID.'/'; 
					break; 
				...
				case "video":
					$Dir = '/newHomePage/upload/video/'. $userType . '/'. $userType . '_'.$iLoginID.'/'; 
					break; 
...
/************************************************ Multiple File Upload ************************************************/
	function handleMultipleFileUpload($userType, $fileType, $iLoginID, $fileArray) {
		...
		$Dir = '/newHomePage/upload/puwHostImgs/'. $userType . '/'.$userType.'_' . $iLoginID . '/';
		$target_dir = realpath($_SERVER['DOCUMENT_ROOT']) . $Dir;
...
/************************************************ Change time format ************************************************/
	function changeTimeFormat($time,$toDB){
		...
		if($toDB){
			...
			$newTime = date_format($newTime, 'g:ia');
		}
		return $newTime;
	}
...
/********************************** END - Validate and Sanitize form data **********************************/
?>
