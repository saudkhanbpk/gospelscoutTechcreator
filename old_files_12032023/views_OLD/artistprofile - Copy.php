<?php require_once 'http://localhost/gospelscout/include/header.php';?>
<script>
	...
	extract($_POST)	;
		...
		if($roll != "" && $roll == 'group'){
			...
			if(count($iRollIDGrp) > 0){
				...
				if($_FILES["giftImgGrp".$key]['name'] != ''){
				
				//deleteImage($_POST['extrImageGrp'.$key],'upload/artist/'.$_POST['sGiftGrp'.$key]);
				
				$randno = rand(0,5000);
				$img = $_FILES["giftImgGrp".$key]['name'];
				$giftImage = $randno.$img;
				...
				move_uploaded_file($_FILES["giftImgGrp".$key]['tmp_name'],$folderPath."/".$giftImage);			
			
			}else{
				$giftImage = $_POST['extrImageGrp'.$key]	;
			}
			...
			$field = array("sGroupType","iNumberMembers","sGropName","sMemberName","sGiftName","sImage","sVideo","sYoutubeUrl");
				$value = array($roll,$iNumberMembers,$sGroupName,$_POST['membe'.$key],$_POST['sGiftGrp'.$key],$giftImage,$giftImage2,$_POST['ytGrp'.$key]);	
