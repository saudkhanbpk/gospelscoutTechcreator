...
<?php 
	if(isset($_GET['back'])){
		...
		if(isset($_GET['church'])){
			$url = 'views/churchprofile?church='.$_GET['church'];
		}else{
...
<?php /*?>  <h3 style="float:left;">Church-<?php echo ucfirst($objsession->get('gs_user_name'));?>-<?php echo ucwords(str_replace('_',' ',$_GET['cat']));?></h3><?php */?>
  ...
  <!--<form name="frmVideo" id="frmVideo" method="post">-->
        ...
        <div class="col-lg-12 col-mg-12 col-sm-12"> 
			<div class="flowplayer" data-swf="<?php echo URL;?>views/flowplayer.swf">       
  ...
  <source src="<?php echo URL;?>upload/church/video/<?php echo $gistmaster[$gi]['sMinistrieName'].'/'.$vd;?>" type="video/mp4">
		  ...
		  $.ajax({
			...
			success: function (data) {
			  if(data == 'Please check your video'){
				...
				?>
            	<a class="back" style="margin:-30px  -240px;cursor:pointer;" onclick="check();" id="subVideo"><?php if(count($vs) > 0){?>Un Subscribe<?php }else{?>Subscribe<?php } ?></a>
                </div>
                <?php } ?>
                </div>
		...
		$vid ++;
                ...
                <div class="col-lg-12 col-mg-12 col-sm-12 borderbuttom"> 
				<?php 
			   $videocountReview = $obj->fetchRowAll("videocomments","sVideoName = '".$_GET['youtube']."' ORDER BY iCommentID DESC");
			?>  
			<iframe width="492" height="350" src="https://www.youtube.com/embed/<?php echo $_GET['youtube']; ?>" frameborder="0" allowfullscreen></iframe>
		...
		if ( $("#log_id").val() != "") {
			...
			if($('#hideLogID').val() == ''){
				...
				if(comment == ""){
				  ...
				  $.ajax({
					type: 'post',
					url: '<?php echo URL;?>views/insertpost.php?comment='+comment+'&loginid=<?php echo $objsession->get('gs_login_id');?>&videoname=<?php echo $_GET['video'];?>&church=<?php echo $_GET['church'];?>&cat=<?php echo $_GET['cat'];?>&rating='+rating_star+'&userid=<?php echo $ids;?>&youtube=<?php echo $_GET['youtube'];?>',
					data: '',
					success: function (data) {	
					$('#comment').val('');	  
						$('#cm').html(data);			  
    ...
    <div class="row" >
        ...
        <div class="col-lg-4 col-mg-4 col-sm-4" style="float: right;">
			...
			?> days Ago</p>
                ...
                <a style="float: right; margin-right: -100px; color: red;" href="<?php echo URL;?>views/churchvideodetails.php?video=<?php echo $_GET['video'];?>&youtube=<?php echo $_GET['youtube'];?>&church=<?php echo $_GET['church'];?>&cat=<?php echo $_GET['cat'];?>&iCommentID=<?php echo $videocountReview[$vi]['iCommentID'];?>" onclick="return confirm('Are you sure want to delete?');">X</a>
        </div>
        <?php } ?>
    </div>
    </div>
...
<div class="col-lg-7 col-md-7 col-sm-7 p0 col-xs-6">
            ...
            <p style="margin-left:10px; margin-bottom: 4px; ">Views : <?php if($videoReview['iViews'] != ""){ echo $videoReview['iViews'];}else{ echo '0';}?> </p>
...
<div class="col-lg-7 col-md-7 col-sm-7 p0 col-xs-6">
			...
			?>
        	<span id="loadlogin"></span>
            <p style="margin-left:10px; margin-bottom: 4px;" id="uname<?php echo $yu;?>"><?php echo pathinfo($vdiName[$v],PATHINFO_FILENAME);?></p>
            <!--<p style="margin-left:10px; margin-bottom: 4px;"><?php echo ucwords($userRow['sFirstName'].' '.$userRow['sLastName']);?></p>-->
            <p style="margin-left:10px; margin-bottom: 4px; " id="Uviews<?php echo $yu;?>">View : <?php if($videoReview['iViews'] != ""){ echo $videoReview['iViews'];}else{ echo '0';}?> </p>
...
//	$views = $obj->fetchRow("videomaster","sVideoName = '".$_GET['video']."' AND iLoginID = ".$objsession->get('gs_login_id'));
		...
		if(isset($_GET['church']))	{
			...
			redirect(URL.'views/churchvideodetails.php?video='.$_GET['video']."&youtube=".$_GET['youtube']."&cat=".$_GET['cat']);	
		}
}
...
if(isset($_GET['iCommentID'])){
	...
	if($obj->delete('videocomments',"iCommentID = ".$_GET['iCommentID']) == true){
		...
		redirect(URL.'views/churchvideodetails.php?video='.$_GET['video']."&church=".$_GET['church']."&youtube=".$_GET['youtube']."&cat=".$_GET['cat']);	
	}
