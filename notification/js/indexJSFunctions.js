/**** Create function for xml http request ****/
  function useBackbone(getVar,newLocal){
    ...
    /* New XML Http Request */
      ...
      queryBackbone.open('GET','https://www.gospelscout.com/notification/phpBackend/connectToDb.php?'+getVar);
      queryBackbone.send()
  }
...
function displayNotifications(notific_info){
	...
	for(x in notific_info.data){
		for(y in notific_info.data[x]){
		    ...
		    notifications += '<img src="https://www.gospelscout.com/img/gospelscout_logo.png" alt="test" height="40px" width="40px" class="mr-2 rounded">';
			...
			if(notific_info.data[x][y].action !== 'suggestGig'){
				notifications += '"https://www.gospelscout.com/views/artistprofile.php?artist='+notific_info.data[x][y].notifierID+'"';
			}else{
				notifications += '"#"';
			}
			...
			notifications += '<strong class="text-gs text-gray-dark">';
		      if(notific_info.data[x][y].action == 'suggestGig'){
				...
				var notific_action = '<a href="https://www.gospelscout.com/views/artistprofile.php?artist='+notific_info.data[x][y].notifierID+'" target="_blank" class="d-inline text-gs">'+notific_info.data[x][y].sFirstName+' '+notific_info.data[x][y].sLastName+'</a> '+notific_info.data[x][y].notificationDescription;
			  }
...
function displayGigSuggestions(notific_info){
	...
	for(x in notific_info.data){
		    ...
		    notifications += '<a href="https://www.gospelscout.com/publicgigads/ad_details.php?g_id='+notific_info.data[x].gigID+'" class="viewSuggestion" notID="'+notific_info.data[x].id+'" link="'+notific_info.data[x].link+'" class="text-gs">  View</a>';
		    notifications += '</p>';
			notifications += '</div>';
	}
	return notifications;
...
function displayGigSubmissions(notific_info){
	...
	for(x in notific_info.data){
		    ...
		    notifications += '<a href="https://www.gospelscout.com/publicgigads/ad_details.php?g_id='+notific_info.data[x].gigId+'" class="viewSubmission" notID="'+notific_info.data[x].id+'" link="'+notific_info.data[x].link+'" class="text-gs">  View</a>';
		    notifications += '</p>';
			notifications += '</div>';
	}
	return notifications;
...
function displayGigInquiries(notific_info){
	...
	for(x in notific_info.data){
		    ...
		    notifications += '<img src="https://www.gospelscout.com/img/gospelscout_logo.png" alt="test" height="40px" width="40px" class="mr-2 rounded">';
		    notifications += '<p class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray">';
		    notifications += '<a href="https://www.gospelscout.com/views/artistprofile.php?artist='+notific_info.data[x].iLoginID+'" target="_blank" class="d-block">'; 
			notifications += '<strong class="text-gs text-gray-dark">';
		    ...
		    notifications += '<a href="https://www.gospelscout.com/publicgigads/ad_details.php?g_id='+notific_info.data[x].gigId+'" class="viewInquiry" notID="'+notific_info.data[x].id+'" link="'+notific_info.data[x].link+'" class="text-gs">  View</a>';
		    notifications += '</p>';
			notifications += '</div>';
	}
	return notifications;
