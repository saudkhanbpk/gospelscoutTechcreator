...
/************************************************************** Notification function **************************************************************/
	function createNotification($db, $obj, $action, $notifier, $notified, $link){
		...
		/********************* Define a switch statement to determine the link path *********************/
			switch ($action) {
              case 'gigRequest':
                $insertArray['link'] = URL . 'publicgigads/ad_details.php?g_id=' . $link; //views/gigform.php 
                break;

              case 'gigInquiry':
                $insertArray['link'] = URL . 'publicgigads/ad_details.php?g_id=' . $link;
                break;

              case 'suggestGig':
                $insertArray['link'] = URL . 'publicgigads/ad_details.php?g_id=' . $link; //4472756d343938?id=
                break;
							...
							if(count($getSubscribersResults) > 0){
								/* Loop through the subscriber results and insert into the notificationmaster table */
									foreach($getSubscribersResults as $subscriber){
										...
										$notificationInsert = pdoInsert($db,$field,$value,$table);
	...
	function findArtistMatch($db, $obj, $gigInfo){
	    ...
	    /* Create query statement and query database */ 
	        ...
	        /* continue query statement concatenation */
	            $findMatchQuery .= 'WHERE usermaster.sUserType = :sUserType AND usermaster.iLoginID != :gigManLoginId AND usermaster.sAvailability != :sAvailability AND states.name = :sStateName AND usermaster.dDOB <= :dDOB AND ';
	        ...
	        /* Select appropriate column corresponding to the userType */
	            if($gigInfo['userType'] == 'artist'){
					$art_needed = $gigInfo['art_needed'];
					// var_dump($art_needed);
					$numb_art_needed = count($art_needed);
					$ii = 1; 
					foreach($art_needed as $art_needed_index => $art_needed_val){
						// var_dump($art_needed_val['artistType']);
						// var_dump($art_needed_val['tal_pay']);
						if($ii == $numb_art_needed){
							$findMatchQuery .= '(talentmaster.TalentID = :TalentID_'.$art_needed_index.' AND usermaster.minPay <= :minPay_'.$art_needed_index.')';
						}else{
							$findMatchQuery .= '(talentmaster.TalentID = :TalentID_'.$art_needed_index.' AND usermaster.minPay <= :minPay_'.$art_needed_index.') OR ';
						}
						$ii++;
					}
	            }
	            elseif($gigInfo['userType'] == 'group'){
	                $findMatchQuery .= 'usermaster.sGroupType = :sGroupType AND usermaster.minPay <= :minPay';
	            }
	        ...
	        */
	        ...
	        try{
				...
				foreach($art_needed as $art_needed_index => $art_needed_val){
					$findMatch->bindParam(':minPay_'.$art_needed_index,$art_needed_val['tal_pay']);
				}
	            ...
	            /* Select appropriate bindParam corresponding to the userType */
		            if($gigInfo['userType'] == 'artist'){
						foreach($art_needed as $art_needed_index => $art_needed_val){
							$findMatch->bindParam(':TalentID_'.$art_needed_index,$art_needed_val['artistType']);
						}
		            }
		            elseif($gigInfo['userType'] == 'group'){
...
/******************************************* Query Database with PDO Method *************************************/
	...
	*/
	...
	function pdoQuery($table,$columnsArray,$paramArray,$orderByParam,$innerJoinArray,$leftJoinArray,$paramOrArray,$groupByParam,$limit){
		...
		/* INNER JOIN & LEFT JOIN */
			...
			if( count($leftJoinArray) > 0 ){
				$leftJoin = '';
				for($p=0;$p<count($leftJoinArray);$p++){
					$innerJoin .= ' LEFT JOIN ';
					$innerJoin .= $leftJoinArray[$p][0] . ' ' . $leftJoinArray[$p][0] . ' on ' . $leftJoinArray[$p][2] . '.' . $leftJoinArray[$p][3] . ' = '. $leftJoinArray[$p][0] . '.' . $leftJoinArray[$p][1];
				}
			}
			...
			/* Nested foreach loops allow user of function to specify the condition and more specifically the operator being used (>,<,=,etc) */
				foreach($paramArray as $index1 => $value1){
					foreach($value1 as $index1_5 => $value1_5){
						if($j == $loopMax1){
							if($index1 == 'postedgigsmaster.MONTH(gigDate)'){
								$cond .= 'MONTH(gigDate) '. $index1_5 . ' :getMonth';
							}
							else{
							...
							if($index1 == 'postedgigsmaster.MONTH(gigDate)'){
								$cond .= 'MONTH(gigDate) '. $index1_5 . ' :getMonth AND ';
							}
							else{
		...
		/**************************************** Add OR params ****************************************/
			...
			if( count($paramOrArray) > 0 ){
				$b_param_id = '';
				$cond .= ' AND (';
				$loopMax2 = 0;
				$k = 1; 

				foreach($paramOrArray as $index2 => $value2){
					foreach($value2 as $index2_5 => $value2_5){
						
						$loopMax2 = count($value2_5);
						foreach($value2_5 as $value2_5_index => $value2_5_val){
							$b_param_id = $index2.'_'.$value2_5_index;
							if($k == $loopMax2){
								if($index2 == 'postedgigsmaster.MONTH(gigDate)'){
									$cond .= 'MONTH(gigDate)'. $index2_5 . ' :getMonth';//$index1 .
								}
								else{
									$cond .= $index2 .' '. $index2_5 . ' :' . str_replace('.','',$b_param_id);
								}
							}
							else{
								if($index2 == 'postedgigsmaster.MONTH(gigDate)'){
									$cond .= 'MONTH(gigDate)'. $index2_5 . ' :getMonth OR ';//$index1 .
								}
								else{
									$cond .= $index2 .' '. $index2_5 . ' :' . str_replace('.','',$b_param_id) . ' OR ';
								}
							}

							if($index2 == 'postedgigsmaster.MONTH(gigDate)'){
								$bParam[] = ':getMonth,' . $value2_5_val;
							}
							else{
								$bParam[] = ':' . str_replace('.','',$b_param_id) . ',' . $value2_5_val;
							}
							$k++;
						}
					}
		...
		/* Define Query Statement */
			...
			if($groupByParam){
				$queryStatement .= ' GROUP BY ' . $groupByParam;
			}

			if($limit){
				$queryStatement .= ' LIMIT ' . $limit['offset'] . ', ' . $limit['no_of_records'];
			}
